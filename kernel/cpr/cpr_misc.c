...
cpr_alloc_statefile(int alloc_retry)
{
	...
	if (alloc_retry) {
		...
		if (cpr_debug & (CPR_DEBUG1 | CPR_DEBUG7))
			prom_printf(str);
		if (C_VP->v_type != VBLK)
			(void) fop_dumpctl(C_VP, DUMP_FREE, NULL, NULL);
	} else {
		/*
	...
	if (C_VP->v_type != VBLK) {
		 ...
		 */
		if (rc = fop_dumpctl(C_VP, DUMP_ALLOC, NULL, NULL)) {
...
cpr_statef_close(void)
{
	if (C_VP) {
		if (!cpr_reusable_mode)
			(void) fop_dumpctl(C_VP, DUMP_FREE, NULL, NULL);
		(void) fop_close(C_VP, FWRITE, 1, (offset_t)0, CRED(), NULL);
		VN_RELE(C_VP);
		C_VP = 0;
	}
}
...
cpr_write_deffile(cdef_t *cdef)
{
	...
	if (rc = cpr_rdwr(UIO_WRITE, vp, cdef, sizeof (*cdef)))
		str = "write";
	else if (rc = fop_fsync(vp, FSYNC, CRED(), NULL))
		str = "fsync";
