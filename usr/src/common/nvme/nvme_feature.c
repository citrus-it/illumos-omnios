 ...
 * honor the original names. Most fields here try to use a 0 value as the
 * default value that is ture for most pieces. For example, NVME_FEAT_MANDATORY,
 * NVME_FEAT_CSI_NONE, etc. are all the defaults so that way we are generally
 * only encoding exceptions.
 */
