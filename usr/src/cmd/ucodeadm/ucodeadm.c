...
ucode_convert_amd(const char *infile, uint8_t **bufp, int *sizep)
{
	...
	*sizep = size;
	 ...
	 * what we can check here but we will never use a patch which has a
	 * value in a North/South bridge restriction field, and we can limit
	 * this to plausible CPU revisions and things that look like they have
	 * a valid date field. The date field is encoded as a kind of packed
	 * BCD 0xxMMDDYYYY date.
	 */
	patch = (ucode_header_amd_t *)*bufp;
	month = bcd_to_int(bitx32(patch->uh_date, 31, 24));
	day = bcd_to_int(bitx32(patch->uh_date, 23, 16));
	yearh = bcd_to_int(bitx32(patch->uh_date, 15, 8));
