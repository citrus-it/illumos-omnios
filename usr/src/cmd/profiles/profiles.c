...
static int show_profs(const char *, const char *, int, int);
...
main(int argc, char *argv[])
{
	...
	if (*argv == NULL) {
		...
		do {
			(void) printf("%s:", *argv);
			(void) printf("\n");
			if (show_profs(*argv, cmd, print_flag,
			    search_flag) != 0) {
				status = EXIT_FAILURE;
			}
			if (argv[1] != NULL) {
...
/*
 * The authenticated and unauthenticated profile sets are enumerated
 * separately, and a profile may appear in both, so a list of the
 * profiles already reported is kept to avoid showing duplicates.
 */
static bool
seen_profile(const callback_t *call, const char *prof)
{
	for (uint_t i = 0; i < call->nseen; i++) {
		if (strcmp(call->seen[i], prof) == 0)
			return (true);
	}
	return (false);
}
...
static void
remember_profile(callback_t *call, const char *prof)
{
	char **n;

	n = realloc(call->seen, (call->nseen + 1) * sizeof (char *));
	if (n == NULL)
		return;
	call->seen = n;
	call->seen[call->nseen] = strdup(prof);
	if (call->seen[call->nseen] != NULL)
		call->nseen++;
}

static void
forget_profiles(callback_t *call)
{
	for (uint_t i = 0; i < call->nseen; i++)
		free(call->seen[i]);
	free(call->seen);
    ...
    void *arg __unused)
{
	...
	if (seen_profile(call, prof))
		return (0);
	remember_profile(call, prof);
...
print_profs_long(execattr_t *exec, callback_t *call)
{
	...
	for (curprofile = ""; exec != NULL; exec = exec->next) {
		...
		if (strcmp(curprofile, exec->name) != 0) {
			...
			skip = seen_profile(call, curprofile);
			if (skip)
				continue;
			remember_profile(call, curprofile);
...
print_profs_short(execattr_t *exec, callback_t *call)
{
	...
	for (curprofile = ""; exec != NULL; exec = exec->next) {
		...
		if (strcmp(curprofile, exec->name) != 0) {
			curprofile = exec->name;
			if (seen_profile(call, curprofile))
				continue;
			remember_profile(call, curprofile);
			if ((call->print_flag & PRINT_VERBOSE) && call->auth) {
				(void) printf("      %s (%s)\n", curprofile,
