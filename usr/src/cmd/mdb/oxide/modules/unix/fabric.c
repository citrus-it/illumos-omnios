...
i_pcie_core(uintptr_t addr, const void *arg __unused, void *cb_data)
{
	...
	mdb_printf("%?p %4r %4r %4r %4r %5r %-9s %s\n", addr, core.zpc_coreno,
    ...
    const mdb_arg_t *argv)
{
	...
	if (!(flags & DCMD_PIPE_OUT) && DCMD_HDRSPEC(flags)) {
		mdb_printf("%<u>%?s %4s %4s %4s %4s %5s %-9s %s%</u>\n",
		    "ADDR", "NUM", "IOMS", "IOHC", "BUS", "PORTS", "LANES",
		    "FLAGS");
	}
...
fabric_nbif_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (!(flags & DCMD_PIPE_OUT) && DCMD_HDRSPEC(flags)) {
		mdb_printf("%<u>%?s %4s %4s %4s %5s%</u>\n",
		    "ADDR", "NUM", "IOMS", "BUS", "FUNCS");
	}
...
/*
 * The CPU side of the tree. Each level is identified by its logical number and
 * the physical number it maps to.
 */
typedef struct {
	uint_t		fcp_flags;
	uint64_t	fcp_num;
...
fabric_core_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	return (fabric_cpu_dcmd(addr, flags, argc, argv, "zen_core",
	    i_zen_core, "  CCD  CCX  NUM PHYS THREADS APICIDS"));
}
...
fabric_levels_init(void)
{
	for (uint_t i = FABRIC_L_ROOT + 1; i < FABRIC_L_NLEVELS; i++) {
		...
		if (mdb_ctf_lookup_by_name(ld->fld_ptype, &tid) != 0 ||
		    ...
		    (sz = mdb_ctf_type_size(mid)) <= 0 ||
		    sz > sizeof (uint64_t) || off % NBBY != 0) {
			mdb_warn("failed to find %s::%s\n", ld->fld_ptype,
			    ld->fld_count);
			return (false);
		}
...
fabric_collect_push(fabric_collect_t *c, uintptr_t addr)
{
	if (c->fc_n == c->fc_cap) {
		...
		na = mdb_alloc(ncap * sizeof (uintptr_t), UM_SLEEP);
		if (c->fc_addrs != NULL) {
...
{
	fabric_level_t path[FABRIC_L_NLEVELS], l;
	uint_t depth = 0, i;
	GElf_Sym sym;
	 ...
	 * collected.
	 */
	for (l = c->fc_level; l != FABRIC_L_ROOT;
...
static void
fabric_collect_free(fabric_collect_t *c)
{
	if (c->fc_addrs != NULL)
		mdb_free(c->fc_addrs, c->fc_cap * sizeof (uintptr_t));
	mdb_free(c, sizeof (*c));
...
fabric_walk_init(mdb_walk_state_t *wsp)
{
	...
	c = mdb_zalloc(sizeof (*c), UM_SLEEP);
	c->fc_level = level;
	c->fc_start = wsp->walk_addr;

	if (!fabric_collect(c)) {
		fabric_collect_free(c);
		return (WALK_ERR);
	}
...
void
fabric_walk_fini(mdb_walk_state_t *wsp)
{
	fabric_collect_free(wsp->walk_data);
}
