...
/*
 * Print the summary line for a port. This shows its most recently captured
 * state, whether link-up or link-down events were captured and, if requested,
 * its live state. The live state comes last since it is read from the
 * hardware and is independent of the captures.
 */
static void
    ...
    const ltssm_reg_t *lc0, int latest)
{
	...
	if (latest >= 0)
		nm = ltssm_state_name(cb, lc0->lr_val[latest]);
	(void) mdb_snprintf(bdf, sizeof (bdf), "%r/%r/%r",
	    lp->lp_busno, lp->lp_device, lp->lp_func);
	mdb_printf("%?p %-8s %-22s", lp->lp_addr, bdf, nm);
...
/*
 * Print the detailed view of a port. This is the decoded history from each
 * capture point, followed by the live history if requested.
 */
static void
...
ltssm_port_cb(uintptr_t addr, const void *arg __unused, void *cbdata)
{
	...
	/*
	 * In the summary, ports for which nothing has been captured are only
	 * of interest when their live state is being read.
	 */
	if (latest < 0 && !cb->lc_live && !cb->lc_detail)
		return (WALK_NEXT);
...
ltssm_dcmd_help(void)
{
	mdb_printf(
	    ...
	    "captures. Within kmdb, the -l option additionally reads the\n"
	    "current state of each port from the hardware, adding a LIVE\n"
	    "column to the summary and a LIVE section to the detailed output.\n"
	    "The CAPTURED and EVENTS columns describe the kernel's captures.\n"
	    "LIVE is read from the hardware when the command runs and is\n"
	    "independent of them.\n"
	    "\n%<b>Options:%</b>\n"
	    "\t-l\tread the live LTSSM state from the hardware (kmdb only).\n");
}
...
ltssm_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (flags & DCMD_ADDRSPEC) {
		...
		if (cb.lc_live) {
			mdb_printf("%<u>%?s %-8s %-22s %-8s %s%</u>\n",
			    "ADDR", "B/D/F", "CAPTURED", "EVENTS", "LIVE");
		} else {
