 ...
 * state, its live state if requested, and whether link-up or link-down
    ...
    const ltssm_reg_t *lc0, int latest)
{
	...
	if (lc0 != NULL && lc0->lr_ts[l->ll_link_up] != 0)
		mdb_printf(" up");
	if (lc0 != NULL && lc0->lr_ts[l->ll_link_down] != 0)
		mdb_printf(" down");
	mdb_printf("\n");
}
...
ltssm_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (live != 0) {
		...
		if (cb.lc_lc_state_reg == NULL) {
			mdb_warn("live LTSSM state is not supported on the "
	...
	if (flags & DCMD_ADDRSPEC) {
		...
		if (cb.lc_live) {
			mdb_printf("%<u>%?s %-8s %-22s %-22s %s%</u>\n",
			    "ADDR", "B/D/F", "CAPTURED", "LIVE", "EVENTS");
		} else {
