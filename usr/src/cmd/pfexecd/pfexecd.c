...
/*
 * The maximum number of authentication conversations that may be in progress
 * at any one time. Each one occupies a door server thread and a helper process
 * for up to the helper's timeout. It must be below DOOR_THREADS, so that
 * pending authentication conversations can never starve the fixed thread pool
 * and block ordinary requests.
 */
...
/*
 * Answer a request which is being rejected.
 */
static void
...
/*
 * Spawn the pfauth helper with the terminal on its standard file descriptors
 * and no other descriptors open. On success, stores the helper's process ID
 * and returns 0.
 */
static int
spawn_pfauth(int ttyfd, char *const *argv, pid_t *pidp)
{
	...
	/*
	 * The helper must not signal this process when it exits, and may only
	 * be reaped by a wait for its specific process ID.
	 */
	if ((ret = posix_spawn_file_actions_adddup2(
	    &fa, ttyfd, STDIN_FILENO)) == 0 &&
	    (ret = posix_spawn_file_actions_adddup2(
	    &fa, ttyfd, STDOUT_FILENO)) == 0 &&
	    (ret = posix_spawn_file_actions_adddup2(
	    &fa, ttyfd, STDERR_FILENO)) == 0 &&
	    (ret = posix_spawn_file_actions_addclosefrom_np(
	    &fa, STDERR_FILENO + 1)) == 0 &&
	    (ret = posix_spawnattr_setflags(
	    &attr, POSIX_SPAWN_WAITPID_NP | POSIX_SPAWN_NOSIGCHLD_NP)) == 0) {
		ret = posix_spawn(pidp, PFAUTH_PATH, &fa, &attr, argv,
		    environ);
	}
...
/*
 * Run the pfauth helper with the terminal on the standard file descriptors and
 * wait for it to complete. Returns true if the helper ran and exited
 * successfully.
 */
static bool
...
{
	char *const argv[] = {
		"pfauth", (char *)arg1, (char *)arg2, (char *)arg3, NULL
	};
	pid_t pid;
	int stat = -1;
	int ret;

	if ((ret = spawn_pfauth(ttyfd, argv, &pid)) != 0) {
		syslog(LOG_ERR, "could not run authentication helper: %s",
		    strerror(ret));
		return (false);
	}
...
/*
 * Discard cached authentication state, for the calling user and terminal.
 */
static void
