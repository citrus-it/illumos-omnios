...
{
	switch (resp) {
	case UHSMP_RESPONSE_INCOMPLETE:
		return ("incomplete");
	case UHSMP_RESPONSE_OK:
		return ("success");
	case UHSMP_RESPONSE_INVALID_ARGS:
		return ("rejected - invalid arguments");
	case UHSMP_RESPONSE_INVALID_MSGID:
		return ("failure - invalid/unsupported message ID");
	case UHSMP_RESPONSE_REJECTED_PREREQ:
		return ("rejected - missing pre-requisite(s)");
	case UHSMP_RESPONSE_REJECTED_BUSY:
...
uhsmp_parse_uint32(const char *str, uint32_t *valp)
{
	...
	if (errstr != NULL) {
		warnx("value %s (range 0-%u): %s", errstr, UINT32_MAX, str);
		return (false);
	}
	*valp = l;
	return (true);
