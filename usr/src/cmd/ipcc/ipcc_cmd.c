...
static uint8_t *
ipcc_readfile(const char *filename, size_t *lenp)
{
	uint8_t *buf, *readbuf;
	size_t remaining;
	struct stat st;
	...
	if (buf == NULL) {
		err(EXIT_FAILURE, "failed to allocate 0x%" PRIx64 " bytes for "
		    "reading file %s", st.st_size, filename);
	}
...
ipcc_keyset(int argc, char *argv[])
{
	...
	buf = ipcc_readfile(filename, &len);

	if (!libipcc_keyset(ipcc_handle, key, buf, len, flags))
...
ipcc_rot(int argc, char *argv[])
{
	...
	buf = ipcc_readfile(argv[0], &len);

	if (!libipcc_rot(ipcc_handle, buf, len, &reply))
		libipcc_fatal("Failed to perform RoT operation");
	free(buf);

	printf("Success\n");

	buf = libipcc_rot_buf(reply, &len);
	hexdump(buf, len);
	libipcc_rot_free(reply);
