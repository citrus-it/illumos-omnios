...
static void
ipcc_apobwrite_usage(FILE *f)
{
	(void) fprintf(f, "\tapobwrite <filename>\n");
}

static int
ipcc_apobwrite(int argc, char *argv[])
{
	uint8_t *data;
	size_t len;
	...
	if (!libipcc_apobwrite(ipcc_handle, data, len))
...
static const ipcc_cmdtab_t ipcc_cmds[] = {
	...
	{ "apobwrite", ipcc_apobwrite, ipcc_apobwrite_usage },
