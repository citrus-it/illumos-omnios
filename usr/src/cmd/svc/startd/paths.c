 ...
 * which applies to the start method. Symbolic links are rejected at
 * every component of the path so that this code, which runs with full
    ...
    uint_t index, char **valp)
{
	*valp = NULL;
	...
	(void) snprintf(ctx->mpc_pname, sizeof (ctx->mpc_pname), fmt, index);

	if (scf_pg_get_property(pg, ctx->mpc_pname, ctx->mpc_prop) != 0) {
		if (scf_error() == SCF_ERROR_NOT_FOUND)
			return (0);
		log_instance(ctx->mpc_inst, B_TRUE, "Could not read managed "
		    "path property '%s': %s.", ctx->mpc_pname,
	...
	if (scf_property_get_value(ctx->mpc_prop, ctx->mpc_val) != 0 ||
	    ...
	    ctx->mpc_bufsz) < 0) {
		log_instance(ctx->mpc_inst, B_TRUE, "Managed path property "
		    "'%s' is invalid: %s.", ctx->mpc_pname,
		    scf_strerror(scf_error()));
		return (-1);
	}
...
/*
 * Parse a numeric user or group id in text form. Returns true if the
 * string was parsed in full and the result is no greater than maxid.
 */
static bool
mp_parse_id(const char *str, unsigned long maxid, unsigned long *idp)
{
	char *end;

	errno = 0;
	*idp = strtoul(str, &end, 10);

	return (errno == 0 && end != str && *end == '\0' && *idp <= maxid);
}
...
mp_resolve_user(const mp_ctx_t *ctx, const char *str, uid_t *uidp)
{
	...
	if (isdigit((uchar_t)str[0])) {
		...
		if (!mp_parse_id(str, UID_MAX, &id)) {
			log_instance(ctx->mpc_inst, B_TRUE, "Invalid user "
			    "\"%s\" for managed path.", str);
			return (-1);
		}
		*uidp = (uid_t)id;
		return (0);
	...
	if (pwp == NULL) {
		startd_free(buf, bufsz);
		log_instance(ctx->mpc_inst, B_TRUE, "Managed path user "
		    "\"%s\" was not found.", str);
		return (-1);
	}
...
mp_resolve_group(const mp_ctx_t *ctx, const char *str, gid_t *gidp)
{
	...
	if (isdigit((uchar_t)str[0])) {
		...
		if (!mp_parse_id(str, GID_MAX, &id)) {
			log_instance(ctx->mpc_inst, B_TRUE, "Invalid group "
			    "\"%s\" for managed path.", str);
			return (-1);
		}
		*gidp = (gid_t)id;
	...
	if (grp == NULL) {
		startd_free(buf, bufsz);
		log_instance(ctx->mpc_inst, B_TRUE, "Managed path group "
		    "\"%s\" was not found.", str);
		return (-1);
	}
 ...
 * entries are unlinked while the directory is being read, so a scan can
 * miss entries and the directory is rescanned until a pass finds nothing
 * left to remove. The number of passes is bounded so that a concurrent
 * writer cannot block this loop, and the instance, indefinitely. Entries
 * added by others while cleaning is in progress may remain.
 */
static int
mp_empty_dir(int dirfd, uint_t depth)
{
	...
	for (pass = 0; pass < MP_EMPTY_MAXPASSES && err == 0; pass++) {
		...
		while (err == 0 && (dp = readdir(dirp)) != NULL) {
			 ...
			 * it is a concurrent addition; leave it to the best
			 * effort rules rather than failing.
			 */
 ...
 * and permissions to the final component. The walk moves forward one
 * component at a time using openat(2) with O_NOFOLLOW so that symbolic
 * links are rejected wherever they appear. When empty is set and the
    ...
    mode_t mode, bool empty)
{
	...
	if (path[0] != '/') {
		log_instance(ctx->mpc_inst, B_TRUE, "Managed path \"%s\" is "
		    "not an absolute path.", path);
		return (-1);
	}

	if ((fd = open("/", O_RDONLY | O_DIRECTORY)) < 0) {
		log_instance(ctx->mpc_inst, B_TRUE, "Could not open the root "
	...
	for (; comp != NULL; comp = next) {
		 ...
		 * intermediates are pinned to root:root 0755 in case the
		...
		if (final) {
			if (fchown(fd, uid, gid) != 0 ||
			    fchmod(fd, mode) != 0) {
				log_instance(ctx->mpc_inst, B_TRUE, "Could "
				    "not set ownership or permissions on "
	...
	if (empty && !created) {
		...
		fd = -1;
		if (err != 0) {
			log_instance(ctx->mpc_inst, B_TRUE, "Could not empty "
			    "managed path \"%s\": %s.", path, strerror(err));
			goto fail;
		}
	}
    ...
    const char *path)
{
	...
	/*
	 * The environment array is grown and freed with the same
	 * allocators that librestart's context code uses, since it is
	 * released by restarter_free_method_context().
	 */
	if (mcp->env == NULL) {
		mcp->env_sz = 10;
...
nomem:
	log_instance(ctx->mpc_inst, B_TRUE, "Could not add \"%s\" to the "
	    "method environment: %s.", name, strerror(ENOMEM));
	return (-1);
}
    ...
    scf_snapshot_t *snap, struct method_context *mcp, bool *foundp)
{
	...
	*foundp = true;
	...
	if (expand_method_tokens(rawpath, ctx->mpc_inst->ri_m_inst, snap,
	    METHOD_START, &path) != 0) {
		log_instance(ctx->mpc_inst, B_TRUE, "Could not expand "
		    "managed path \"%s\": %s.", rawpath,
	...
	if (modestr != NULL) {
		...
		m = strtoul(modestr, NULL, 8);
		if (c == modestr || *c != '\0' || m > 07777) {
			log_instance(ctx->mpc_inst, B_TRUE, "Invalid mode "
			    "\"%s\" for managed path \"%s\".", modestr, path);
			goto out;
		}
		mode = (mode_t)m;
	}
 ...
 * receives any environment variable additions, from where set_smf_env()
    ...
    const char *mname, const char *method, struct method_context **mcpp)
{
	 ...
	 * are walked; the instance level is the base level and the
	...
	if (snap != NULL) {
		...
		for (;;) {
			...
			r = scf_snaplevel_get_next_snaplevel(lvl, nlvl);
			if (r != 0) {
				if (scf_error() == SCF_ERROR_NOT_FOUND)
					break;
				log_instance(inst, B_TRUE, "Could not read "
		...
		if (scf_instance_get_parent(scf_inst, svc) != 0) {
			log_instance(inst, B_TRUE, "Could not get the parent "
			    "service: %s.", scf_strerror(scf_error()));
