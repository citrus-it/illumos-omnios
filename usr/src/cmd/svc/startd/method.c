 ...
 * non-persistent, it does not survive a reboot and the method will run again on
 * next boot in case the condition that prompted the service to disable itself
 * has since cleared.
 */
static void
	...
	for (;;) {
		...
		case EROFS:
			log_error(LOG_WARNING, "%s: could not set the enabled "
			    "override to temporarily disable the service: %s.\n",
			    inst->ri_i.i_fmri, strerror(r));
			return;
...
retry:
	...
	if (r == 0 &&
		 ...
		 * (the queued disable will subsequently take it down). See the
		 * comment above method_temp_disable() for details.
		 */
		if (info->sf_method_type == METHOD_START &&
		    exit_code == SMF_EXIT_TEMP_DISABLE)
