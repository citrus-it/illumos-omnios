 ...
 * from the printf(3C) format fmt and the managed path entry index. The
    ...
    scf_type_t ty, ...)
{
	...
	if (internal_attach_property(pg, p) != 0) {
		uu_die(gettext("Could not add managed path property "
		    "\"%s\".\n"), pname);
	}
}
...
lxml_get_directory(pgroup_t *pg, xmlNodePtr dir, uint_t index)
{
	...
	if (path == NULL || *path == '\0') {
		uu_die(gettext("directory element missing \"path\" "
	...
	if (user != NULL) {
		if (*user == '\0') {
			uu_die(gettext("Empty user for directory "
			    "\"%s\".\n"), path);
		}
	...
	if (group != NULL) {
		if (*group == '\0') {
			uu_die(gettext("Empty group for directory "
			    "\"%s\".\n"), path);
		}
	...
	if (mode != NULL) {
		...
		if (c == mode || *c != '\0' ||
		    strtoul((const char *)mode, NULL, 8) > 07777) {
			uu_die(gettext("Invalid mode \"%s\" for directory "
			    "\"%s\".\n"), mode, path);
		}
	...
	/*
	 * An empty env attribute is a synonym for not placing the path in
