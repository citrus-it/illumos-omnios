...
#
# The reason is stored in the general_ovr/comment property, the same property
# that `svcadm disable -c` uses. Like the temporary disable itself, this
# property is non-persistent and is removed automatically when the instance is
# re-enabled.
#
...
smf_method_exit() {
	...
	# best-effort and the disable does not depend on it.
	svccfg -s "$__smf_method_exit_fmri" \
		addpg general_ovr framework P >/dev/null 2>&1
	svccfg -s "$__smf_method_exit_fmri" \
		setprop general_ovr/comment = astring: "$msg" >/dev/null 2>&1
