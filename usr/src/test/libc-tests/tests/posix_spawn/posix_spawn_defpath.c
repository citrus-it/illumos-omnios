 ...
 * to a default search path which differs depending on whether the caller is
 * privileged. This test verifies that the default is derived from the
 * identity of the /caller/, at the time of the call.
 *
...
spawn_and_reap(const char *desc, const posix_spawnattr_t *attr)
{
	...
	if (ret != 0) {
		warnx("TEST FAILED: %s: posix_spawnp returned %d (%s)",
		    desc, ret, strerror(ret));
		return (false);
	}
...
main(void)
{
	...
	if (spawn_and_reap("privileged default path includes /usr/sbin",
	    NULL)) {
		(void) printf("TEST PASSED: privileged default path "
		    "includes /usr/sbin\n");
	} else {
		ret = EXIT_FAILURE;
	}
