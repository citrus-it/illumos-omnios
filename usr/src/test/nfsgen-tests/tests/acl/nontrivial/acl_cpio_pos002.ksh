...
echo "ASSERTION: Verify that '$CPIO' command supports to archive NFSv4 ACLs & xattrs."
...
for user in root $ACL_STAFF1; do
	...
	while (( i < ${#ops[*]} )); do
		...
		RUN_CHECK usr_exec $RUNAT $file $CP $MYTESTFILE attr.0 \
			|| cleanup $STF_FAIL
		RUN_CHECK usr_exec $RUNAT $dir $CP $MYTESTFILE attr.0 \
			|| cleanup $STF_FAIL
...
# '$CPIO' command succeeds to support NFSv4 ACLs & xattrs.
cleanup $STF_PASS
