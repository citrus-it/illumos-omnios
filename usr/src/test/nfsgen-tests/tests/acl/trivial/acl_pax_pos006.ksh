...
echo "ASSERTION: Verify files include attribute in tar archive and restore with " \
	"tar should succeed."
...
for user in root $ACL_STAFF1; do
	...
	#
	cd $MNTDIR/$TST_DIR
	RUN_CHECK usr_exec $PAX -r -x ustar -@ -f $MNTDIR/$paxout > /dev/null \
		|| cleanup $STF_FAIL
	...
	RUN_CHECK usr_exec $RM -rf * || cleanup $STF_FAIL
	RUN_CHECK usr_exec $TAR xf@ $MNTDIR/$paxout || cleanup $STF_FAIL
	RUN_CHECK cksum_files $MNTDIR/$TST_DIR AFTER_FCKSUM AFTER_ACKSUM \
		|| cleanup $STF_FAIL
...
# Files 'pax tar' archive and restre with 'pax tar' passed.
cleanup $STF_PASS
