...
echo "ASSERTION: Compress will keep file attribute intact after the file is " \
...
for user in root $ACL_STAFF1; do
	...
	RUN_CHECK cksum_files $INI_DIR BEFORE_FCKSUM BEFORE_ACKSUM \
		|| cleanup $STF_FAIL
	RUN_CHECK usr_exec "$COMPRESS -f" $INI_DIR/* || cleanup $STF_FAIL
	RUN_CHECK usr_exec $MV $INI_DIR/* $TST_DIR || cleanup $STF_FAIL
	RUN_CHECK usr_exec $UNCOMPRESS $TST_DIR/* || cleanup $STF_FAIL
