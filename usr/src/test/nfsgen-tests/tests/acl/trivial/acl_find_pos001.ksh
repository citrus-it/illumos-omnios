...
echo "ASSERTION: Verifies ability to find files with attribute with" \
...
for user in root $ACL_STAFF1; do
	...
	while (( i < NUM_FILE )); do
		...
		ff=$(usr_exec $FIND $INI_DIR -type f -name ${f##*/} \
			-xattr -print)
		if [[ $ff != $f ]]; then
			echo "find file containing attribute fail."
			cleanup $STF_FAIL
		fi
		...
		while (( j < NUM_ATTR )); do
			...
			if [[ $fa != $af ]]; then
				echo "find file attribute fail"
				cleanup $STF_FAIL
			fi
			(( j += 1 ))
		done
		(( i += 1 ))
		# find all attribute files of $f
	done
