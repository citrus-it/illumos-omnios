...
echo  "ASSERTION: Verifies that ls doesn't display @ in the file " \
...
for user in root $ACL_STAFF1; do
	...
	while (( i < NUM_FILE )); do
		...
		RUN_CHECK usr_exec $RUNAT $f $RM attribute* || cleanup $STF_FAIL
		RUN_CHECK usr_exec $LS -l $f | $AWK '{print substr($1, 11, 1)}' \
			> $STF_TMPDIR/ls.$$ || cleanup $STF_FAIL $STF_TMPDIR/ls.$$
		ls_attr=$(cat $STF_TMPDIR/ls.$$)
		if [[ $ls_attr == "@" ]]; then
			echo "ls with attribute shouldn't success."
			cleanup $STF_FAIL
		fi
