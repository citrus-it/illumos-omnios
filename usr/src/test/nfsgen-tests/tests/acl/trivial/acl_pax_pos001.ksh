...
echo "ASSERTION: Verify include attribute in pax archive and restore with pax " \
	"should succeed."
...
for user in root $ACL_STAFF1; do
	...
	if [[ ! -d $INI_DIR ]]; then 
		RUN_CHECK usr_exec $MKDIR -m 777 -p $INI_DIR \
			|| cleanup $STF_FAIL
	fi
	...
	#
	if [[ ! -d $TMP_DIR ]]; then 
		RUN_CHECK usr_exec $MKDIR $TMP_DIR || cleanup $STF_FAIL
	fi
...
# Directories pax archive and restre with pax passed.
cleanup $STF_PASS
