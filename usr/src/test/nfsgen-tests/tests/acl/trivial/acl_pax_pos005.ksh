...
echo "ASSERTION: Verify files include attribute in cpio archive and restore with " \
	"cpio should succeed."
...
for user in root $ACL_STAFF1; do
	...
	#
	cd $MNTDIR/$TST_DIR
	RUN_CHECK usr_exec $PAX -r -x cpio -@ -f $MNTDIR/$paxout > /dev/null \
		|| cleanup $STF_FAIL
	...
	RUN_CHECK usr_exec $CPIO -iv@ < $MNTDIR/$paxout > /dev/null \
		|| cleanup $STF_FAIL
	RUN_CHECK cksum_files $MNTDIR/$TST_DIR AFTER_FCKSUM AFTER_ACKSUM \
		|| cleanup $STF_FAIL
...
# Files 'pax cpio' archive and restre with 'pax cpio' passed.
cleanup $STF_PASS
