...
# Restore server delegation setting
if [[ -f $STF_TMPDIR/.SERVER_DELEGATION_OFF ]]; then
	RUN_CHECK RSH root $SERVER \
	    "sharectl set -p server_delegation=off nfs" || exit 1
	rm $STF_TMPDIR/.SERVER_DELEGATION_OFF
fi
