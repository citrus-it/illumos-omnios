...
echo "the file RDONLY as a different user, then client b opens the file"
echo "RDWR to cause a recall, verify client a successfully sends delegreturn"
...
CMD1="cd $MNTDIR && chgusr_exec $DTESTUSER1 $local_prog -R -c -d -o 0 \
	-B \\\"1 1 -1\\\" testfile.$$"
CMD2="cd $MNTDIR && chgusr_exec $DTESTUSER2 $local_prog -R -c -d -o 0 \
	-B \\\"1 1 -1\\\" testfile.$$"
