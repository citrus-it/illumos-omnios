...
# the file is designed for "no delegation" tests on multi-clients
# setup. It it called in the following way:
#
#    nodeleg <testfile> <remote_cmd> <local_cmd>
#
# the script will first create $testfile, and execute $remote_cmd,
# and then execute $local_cmd and verify no delegation is granted.
...
# run remote command background on 2nd client
RSH root $CLIENT2 "$remote_cmd > $SRV_TMPDIR/$NAME.remote_cmd.$$" > $STF_TMPDIR/$NAME.err.$$ 2>&1 &
pid=$!

# make sure remote_cmd is running and ready
wait_now 100 "RSH root $CLIENT2 grep I_am_ready $SRV_TMPDIR/$NAME.remote_cmd.$$" > /dev/null 2>&1
if (( $? != 0 )); then
