...
echo "client b opens a file RDONLY and then closes it, then client a"
echo "opens the file RDONLY, verify client a gets RD delegation"
...
REMOTE_CMD=". $SRV_TMPDIR/delegation/bin/deleg.env && \
	...
	$remote_prog -R -c -o 0 -B \\\"1 1 -1\\\" testfile.$$"
LOCAL_CMD="cd $MNTDIR && chgusr_exec $DTESTUSER1 $local_prog -R -c -d -o 0 \
	-B \\\"1 1 -1\\\" testfile.$$"

DTYPE=$RD

$DIR/getdeleg $TESTFILE "$REMOTE_CMD" "$LOCAL_CMD" $DTYPE 0
