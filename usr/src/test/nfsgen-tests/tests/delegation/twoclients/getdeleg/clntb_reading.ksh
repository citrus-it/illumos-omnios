...
echo "client b is opening a file RDONLY, client a opens it RDONLY,"
echo "verify client a gets RD delegation"
...
REMOTE_CMD=". $SRV_TMPDIR/delegation/bin/deleg.env && \
	...
	$remote_prog -R -c -o 0 -B \\\"1 1 0\\\" testfile.$$"
LOCAL_CMD="cd $MNTDIR && chgusr_exec $DTESTUSER1 $local_prog -R -c -d -o 0 \
	-B \\\"1 1 -1\\\" testfile.$$"

DTYPE=$RD

$DIR/getdeleg $TESTFILE "$REMOTE_CMD" "$LOCAL_CMD" $DTYPE 1
