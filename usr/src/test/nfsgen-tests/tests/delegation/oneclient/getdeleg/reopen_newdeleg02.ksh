...
typeset chg_usr_exec=$STF_SUITE/bin/chg_usr_exec
...
echo "then reopen the file RDWR as a different user, get write delegation"
...
# write test file over NFS as differnt user, check delegation type
cd $MNTDIR
chgusr_exec $DTESTUSER1 "$prog -W -c -d -o 4 -B \"1 1 -1\" testfile.$$" \
...
if [[ $deleg_type -ne $WR ]]; then
	...
	cat $STF_TMPDIR/file_operator.outW.$$
        cleanup $STF_FAIL
fi
