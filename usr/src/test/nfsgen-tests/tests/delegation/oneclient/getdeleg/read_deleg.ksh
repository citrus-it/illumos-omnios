...
function cleanup {
	retcode=$1
	rm -f $MNTDIR/testfile.$$ $STF_TMPDIR/readfile.out.$$
	exit $retcode
}
...
$prog -R -c -d -o 0 -B "1 1 -1" $MNTDIR/testfile.$$ \
	> $STF_TMPDIR/readfile.out.$$ 2>&1
deleg_type=$(grep "return_delegation_type" $STF_TMPDIR/readfile.out.$$ \
	| nawk -F\= '{print $2'})
if [[ $deleg_type -ne $RD ]]; then
	print -u2 "unexpected delegation type($deleg_type) when reading file"
	cat $STF_TMPDIR/readfile.out.$$
	cleanup $STF_FAIL
fi
