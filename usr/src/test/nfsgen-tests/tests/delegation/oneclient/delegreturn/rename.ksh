...
echo "then rename it, check delegation is returned."

cd $MNTDIR/
TESTFILE=testfile.$$
CMD1="chgusr_exec $DTESTUSER1 $prog -R -c -d -o 0 \
	-B \\\"1 1 -1\\\" testfile.$$"

DTYPE=$RD
CMD2="mv testfile.$$ testfile.$$.tmp"

$DIR/delegreturn $TESTFILE "$CMD1" $DTYPE "$CMD2" 0 || return $STF_FAIL

echo "open a file RDWR, get write delegation,"
echo "then rename it, check delegation is returned."
CMD1="chgusr_exec $DTESTUSER1 $prog -W -c -d -o 4 \
	-B \\\"1 1 -1\\\" testfile.$$"
DTYPE=$WR
$DIR/delegreturn $TESTFILE "$CMD1" $DTYPE "$CMD2" 0 || return $STF_FAIL
rm -f testfile.$$.tmp
