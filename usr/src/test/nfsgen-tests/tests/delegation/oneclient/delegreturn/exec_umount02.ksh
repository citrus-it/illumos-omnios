...
# cp exeutable script to SHRDIR
RUN_CHECK copy_file_nodeleg \
	${STF_SUITE}/tests/delegation/bin/endless_scr \
	$MNTDIR/endless_scr.$$ || cleanup $STF_UNRESOLVED 
...
CMD1="$MNTDIR/endless_scr.$$ 0"
DTYPE=$RD
CMD2="/usr/sbin/umount $realMNT"

$DIR/delegreturn $TESTFILE "$CMD1" $DTYPE "$CMD2" || \
	cleanup $STF_FAIL "" "$STF_TMPDIR/$NAME.err.$$ $MNTDIR/endless_scr.$$"
rm -f $STF_TMPDIR/*.err.$$ $MNTDIR/endless_scr.$$
