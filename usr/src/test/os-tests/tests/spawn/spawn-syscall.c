    ...
    uint32_t spsize, const void *sa, uint32_t sasize)
{
	...
	if (errno != wanted) {
		TFAIL(name, "got errno %d (%s), wanted %d", errno,
		    strerror(errno), wanted);
		return;
	}
	TPASS(name);
}
...
t_valid(void)
{
	union {
		spawn_args_t sa;
		char pad[sizeof (spawn_args_t) + 16];
	} au;
...
t_param_fuzz(void)
{
	...
	union {
		spawn_param_t sp;
		char pad[sizeof (spawn_param_t) + 64];
	} pu;
	spawn_param_t *sp = &pu.sp;
	uint32_t sasize = valid_args(&au.sa);
	uint32_t spsize = sizeof (*sp) + 64;
	kfile_attr_t *kfa;
