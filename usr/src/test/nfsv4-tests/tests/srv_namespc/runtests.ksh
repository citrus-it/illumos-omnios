...
#
# control program for NFS server name space tests
#
...
echo "Started NFS server name space tests at [`date`] ..."
echo " "

# Now ready to run the tests
TESTLIST=`egrep -v "^#|^  *$" SNSPC.flist`

(
for t in $TESTLIST
do
	echo $t | egrep "^clnt|share" > /dev/null 2>&1 
	if [ $? -eq 0 ]; then
		# run client tests, must be root
		/suexec ./$t
	else
		$TESTROOT/nfsh $t
	fi
