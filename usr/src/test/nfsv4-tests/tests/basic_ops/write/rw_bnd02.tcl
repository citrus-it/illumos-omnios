...
#	verify reading/writing of a attribute file around the boundary
...
set res [compound {Putfh $bfh; Lookup $env(ATTRFILE); Openattr F; Getfh}]
if {$status != "OK"} {
        putmsg stdout 0 "$TNAME: test setup - getclientid"
	putmsg stderr 0 "\t Test UNRESOLVED: unable to get ATTRFILE filehandle."
	putmsg stderr 1 "\tRes: $res"
	exit $UNRESOLVED
}
set ofh [lindex [lindex $res 3] 2]

# First create a tmp file under ATTRDIR for testing
...
set nfh [basic_open $ofh $TFILE $otype $cid_owner open_sid oseqid status]
if {$nfh == -1} {
...
# Start testing
# --------------------------------------------------------------
# a: Write attr file up to 2G boundary (offset=2g-4K, data=4K) - expect OK
set expcode "OK"
set ASSERTION \
    "Write attr file up to 2G boundary (off=2g-4K, data=4K), expect $expcode"
set tag "$TNAME{a}"
...
# b: Read attr file up to 2G boundary (offset=2g-4K, count=4K) - expect OK
set expcode "OK"
set ASSERTION \
    "Read attr file up to 2G boundary (off=2g-4K, count=4K), expect $expcode"
set tag "$TNAME{b}"
putmsg stdout 0 "$tag: $ASSERTION"
...
# c: Write attr file over 2G boundary (offset=2g-4K, data=8K) - expect OK
set expcode "OK"
set ASSERTION \
    "Write attr file over 2G boundary (off=2g-4K, data=8K), expect $expcode"
set tag "$TNAME{c}"
putmsg stdout 0 "$tag: $ASSERTION"
set count 4096
set off [expr $2G - $count]
...
# d: Read attr file over 2G boundary (offset=2g-4K, count=8K) - expect OK
set expcode "OK"
set ASSERTION \
    "Read attr file over 2G boundary (off=2g-4K, count=8K), expect $expcode"
set tag "$TNAME{d}"
putmsg stdout 0 "$tag: $ASSERTION"
set count 8192
set res [compound {Putfh $nfh; Read $open_sid $off $count; Getattr size}]
