...
#
# control program for WRITE op tests
...
if (( $? == $CIPSO_NFSV4 )); then
	iscipso=1
	# In write_neg03{d}, we need to do "Write" operation in non-global zone
        # for TX test, so add a tcl script to the zone.
	echo "connect $SERVER" > $ZONE_PATH/root/write_neg03
	echo 'set res [compound {Putfh $env(qfh); Lookup $env(tf); \
		Write {0 0} 1 f a $env(data)}]' \
		>> $ZONE_PATH/root/write_neg03
	echo 'puts "$status $res"' >> $ZONE_PATH/root/write_neg03
	echo 'disconnect' >> $ZONE_PATH/root/write_neg03
fi
...
[[ $iscipso == 1 ]] && rm $ZONE_PATH/root/write_neg03
