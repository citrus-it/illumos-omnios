...
set owner 5
set group ""
set st [uid_creat $fh $filename owner group res "root@$domain" "" f]
ckres "Create" $st $expct $res $PASS
if {[get_fh "$ROOTDIRS $filename"] != ""} {
	set res [removev4 $TESTFILE]
	if {$status != "OK"} {
		putmsg stdout 0 "Warning: Could not remove $TESTFILE."
...
set owner ""
set group 5
set st [uid_creat $fh $filename owner group res "" "uucp@$domain" f]
ckres "Create" $st $expct $res $PASS
if {[get_fh "$ROOTDIRS $filename"] != ""} {
	set res [removev4 $TESTFILE]
	if {$status != "OK"} {
