...
# - testing successful creation of other file types.
#  NOTE: All tests here needs the server's FS exported with root permission.
# XXX pending on decision of what to do with such FS ...
...
# Start testing
# --------------------------------------------------------------
# a: create a new blk, expect OK
set expcode "OK"
set LFILE $env(TEXTFILE)
set nlnkf "newlnkf.[pid]"
set ASSERTION "Create a new link to a file, expect $expcode"
set tag $TNAME{a}
putmsg stdout 0 "$tag: $ASSERTION"
set res [compound {Putfh $bfh; Create $nlnkf {{mode 0640}} l $LFILE; 
	Getfh; Getattr type}]
...
set tag $TNAME{b}
putmsg stdout 0 "$tag: $ASSERTION"
set res [compound {Putfh $bfh; Create $nlnkd {{mode 0755}} l $DFILE;
	Getfh; Getattr type}]
set cont [ckres "Create" $status $expcode $res $FAIL]
# verify filehandle from CREATE should be good
set cont [verf_fh [lindex [lindex $res 2] 2] $cont $FAIL]
...
set tag $TNAME{c}
putmsg stdout 0 "$tag: $ASSERTION"
set res [compound {Putfh $bfh; Create $nlnkl {{mode 0644}} l $nlnkd; 
	Getfh; Getattr type}]
set cont [ckres "Create" $status $expcode $res $FAIL]
# verify filehandle from CREATE should be good
set cont [verf_fh [lindex [lindex $res 2] 2] $cont $FAIL]
...
set res [compound {Putfh $bfh; Remove $nlnkf; Remove $nlnkd; Remove $nlnkl;}]
if { "$status" != "OK" } {
