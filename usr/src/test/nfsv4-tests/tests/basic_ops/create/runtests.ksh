...
#
# control program for CREATE op tests
...
if (( $? == $CIPSO_NFSV4 )); then
	iscipso=1
	# In create_neg03{a}, we need to do "Create" operation in non-global zone
	# for TX test, so add a tcl script to the zone.  
	echo "connect $SERVER" > $ZONE_PATH/root/create_neg03
	echo 'set res [compound {Putfh $env(qfh); Create "qd.[pid]" {{mode 777}} d}]' \
		>> $ZONE_PATH/root/create_neg03
	echo 'puts "$status $res"' >> $ZONE_PATH/root/create_neg03
	echo 'disconnect' >> $ZONE_PATH/root/create_neg03
fi
...
[[ $iscipso == 1 ]] && rm $ZONE_PATH/root/create_neg03
