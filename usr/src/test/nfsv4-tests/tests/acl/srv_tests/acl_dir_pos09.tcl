...
#
# a: Test adding a default ACL of (rwx--xrwx) to an existing directory
# b: Test the default ACL settings of (rwx--xrwx) are inherited by sub-dir
# c: Test the default ACL settings of (rwx--xrwx) are inherited by sub-dir
# d: Test adding a default ACL of (rwx-w-rwx) to an existing directory
# e: Test the default ACL settings of (rwx-w-rwx) are inherited by sub-dir
# f: Test the default ACL settings of (rwx-w-rwx) are inherited by sub-dir
# g: Test adding a default ACL of (rwxr--rwx) to an existing directory
# h: Test the default ACL settings of (rwxr--rwx) are inherited by sub-dir
# i: Test the default ACL settings of (rwxr--rwx) are inherited by sub-dir
...
if $IsZFS {
        ...
        # in all sub-assertions, we only change "GROUP@" ACEs
        # so other ACEs (OWNER and EVERYONE) are the same.
        lappend inherited_owner_dir_common_list \
        ...
        lappend inherited_owner_file_common_list        \
                "0 0 $inherited_owner_allow_mask OWNER@"        \
                "0 0 $inherited_owner_deny_mask OWNER@"

        lappend inherited_other_dir_common_list \
        ...
        lappend inherited_other_file_common_list        \
                "0 0 $inherited_other_allow_mask EVERYONE@"     \
                "0 0 $inherited_other_deny_mask EVERYONE@"
}
...
putmsg stderr 1 "$TNAME: new dir ACL : $dir_acl_list"
...
if { $status == "OK" } {
	...
	if { [compare_acl_lists $new_acl_list $dir_acl_list MASK] != 0} {
        	putmsg stderr 0 \
                	"\t Test FAIL: default ACL setup failed."
	} 
}
...
set tag "$TNAME{a}"
set ASSERTION "Test adding a default ACL of (rwx--xrwx) to an existing directory"
putmsg stdout 0 "$tag: $ASSERTION"

set sid {0 0}

# Group - allow x, deny rw
set group_allow_mask [ aclmask [ concat $GENERIC_ALLOW_ACL $POSIX_EXECUTE_ACL ] ]

set group_deny_mask [ aclmask [ concat $GENERIC_DENY_ACL $POSIX_READ_ACL \
$POSIX_WRITE_DIR_ACL ] ]

if $IsZFS {
        set inherited_group_allow_mask $group_allow_mask
        set inherited_group_deny_mask $group_deny_mask
...
set acl_list [lreplace $acl_list 2 2 "1 4b $group_deny_mask GROUP\@"]
set acl_list [lreplace $acl_list 3 3 "0 4b $group_allow_mask GROUP\@"]
set acl_list [lreplace $acl_list 4 4 "1 4b $group_deny_mask GROUP\@"]
...
set tag "$TNAME{b}"
set ASSERTION "Test the default ACL settings of (rwx--xrwx) are inherited by sub-dir"
putmsg stdout 0 "$tag: $ASSERTION"
...
if {$sdfh == $NULL} {
	...
	if $IsZFS {
                set exp_acl [ concat $inherited_owner_dir_common_list \
                        $inherited_acl_dir_group_list_b         \
                        $inherited_other_dir_common_list        \
                        $initial_acl_list]
        } else {
...
# ------------------------------------------------------------------------
# c: Test the default ACL settings of (rwx--xrwx) are inherited by sub-file

set tag "$TNAME{c}"
set ASSERTION "Test the default ACL settings of (rwx--xrwx) are inherited by sub-file"
putmsg stdout 0 "$tag: $ASSERTION"
...
if {$sffh == $NULL } {
	...
	if $IsZFS {
        	# The file should have inherited the parent directorys ACL
                set exp_acl [ concat $inherited_owner_file_common_list \
                        $inherited_acl_file_group_list_c         \
                        $inherited_other_file_common_list        \
                        $initial_acl_list]
        } else {
...
set tag "$TNAME{d}"
set ASSERTION "Test adding a default ACL of (rwx-w-rwx) to an existing directory"
putmsg stdout 0 "$tag: $ASSERTION"

set sid {0 0}

# Group - allow w, deny r/x
set group_allow_mask [ aclmask [ concat $GENERIC_ALLOW_ACL $POSIX_WRITE_DIR_ACL ] ]

set group_deny_mask [ aclmask [ concat $GENERIC_DENY_ACL $POSIX_READ_ACL $POSIX_EXECUTE_ACL ] ]
...
if $IsZFS {
	...
	# For case e, ONE ACE on parent directory can create TWO ACEs on sub-dirs
        # As we only concern the mask which will be compared, here we just set
        # both TYPE and FLAG fields with 0.
        lappend inherited_acl_dir_group_list_e  \
                "0 0 $group_deny_mask GROUP\@"  \
                "0 0 $inherited_group_deny_mask  GROUP\@"       \
                "0 0 $group_allow_mask GROUP\@"   \
                "0 0 $inherited_group_allow_mask GROUP\@"       \
	...
	# For case f, ONE ACE on parent directory can create ONE ACE on sub-file
        lappend inherited_acl_file_group_list_f \
                "0 0 $inherited_group_deny_mask  GROUP\@"       \
                "0 0 $inherited_group_allow_mask GROUP\@"       \
                "0 0 $inherited_group_deny_mask  GROUP\@"
}
...
set tag "$TNAME{e}"
set ASSERTION "Test the default ACL settings of (rwx-w-rwx) are inherited by sub-dir"
putmsg stdout 0 "$tag: $ASSERTION"
...
if {$sdfh == $NULL} {
	...
	if $IsZFS {
                set exp_acl [ concat $inherited_owner_dir_common_list \
                        $inherited_acl_dir_group_list_e         \
                        $inherited_other_dir_common_list        \
                        $initial_acl_list]
        } else {
        	set exp_acl [concat $acl_list $acl_list]
...
# ------------------------------------------------------------------------
# f: Test the default ACL settings of (rwx-w-rwx) are inherited by sub-file

set tag "$TNAME{f}"
set ASSERTION "Test the default ACL settings of (rwx-w-rwx) are inherited by sub-file"
putmsg stdout 0 "$tag: $ASSERTION"
...
if {$sffh == $NULL } {
	...
	if $IsZFS {
        	# The file should have inherited the parent directorys ACL
                set exp_acl [ concat $inherited_owner_file_common_list \
                        $inherited_acl_file_group_list_f         \
                        $inherited_other_file_common_list        \
                        $initial_acl_list]
        } else {
        	# The file should have inherited the parent directorys default
...
set tag "$TNAME{g}"
set ASSERTION "Test adding a default ACL of (rwxr--rwx) to an existing directory"
putmsg stdout 0 "$tag: $ASSERTION"

set sid {0 0}

# Group - allow r, deny wx
set group_allow_mask [ aclmask [ concat $GENERIC_ALLOW_ACL $POSIX_READ_ACL ] ]

set group_deny_mask [ aclmask [ concat $GENERIC_DENY_ACL $POSIX_WRITE_DIR_ACL \
$POSIX_EXECUTE_ACL ] ]
...
if $IsZFS {
	...
	# For case h, ONE ACE on parent directory can create TWO ACEs on sub-dirs
        # As we only concern the mask which will be compared, here we just set
        # both TYPE and FLAG fields with 0.
        lappend inherited_acl_dir_group_list_h  \
                "0 0 $group_deny_mask GROUP\@"  \
                "0 0 $inherited_group_deny_mask  GROUP\@"       \
                "0 0 $group_allow_mask GROUP\@"   \
                "0 0 $inherited_group_allow_mask GROUP\@"       \
	...
	# For case i, ONE ACE on parent directory can create ONE ACE on sub-file
        lappend inherited_acl_file_group_list_i \
                "0 0 $inherited_group_deny_mask  GROUP\@"       \
                "0 0 $inherited_group_allow_mask GROUP\@"       \
                "0 0 $inherited_group_deny_mask  GROUP\@"
}
...
set tag "$TNAME{h}"
set ASSERTION "Test the default ACL settings of (rwxr--rwx) are inherited by sub-dir"
putmsg stdout 0 "$tag: $ASSERTION"
...
if {$sdfh == $NULL} {
	...
	if $IsZFS {
                set exp_acl [ concat $inherited_owner_dir_common_list \
                        $inherited_acl_dir_group_list_h         \
                        $inherited_other_dir_common_list        \
                        $initial_acl_list]
        } else {
        	set exp_acl [concat $acl_list $acl_list]
...
# ------------------------------------------------------------------------
# i: Test the default ACL settings of (rwxr--rwx) are inherited by sub-file

set tag "$TNAME{i}"
set ASSERTION "Test the default ACL settings of (rwxr--rwx) are inherited by sub-file"
putmsg stdout 0 "$tag: $ASSERTION"
...
if {$sffh == $NULL } {
	...
	if $IsZFS {
        	# The file should have inherited the parent directorys ACL
                set exp_acl [ concat $inherited_owner_file_common_list \
                        $inherited_acl_file_group_list_i         \
                        $inherited_other_file_common_list        \
                        $initial_acl_list]
        } else {
        	# The file should have inherited the parent directorys default
