...
#
# a: Test server accepts ACE4_READ_NAMED_ATTRS but doesn't return it - expect OK
# b: Test restoring file owner perms - expect OK
# c: Test server accepts ACE4_WRITE_NAMED_ATTRS but doesn't return it - expect OK
# d: Test restoring file owner perms - expect OK
# e: Test server accepts ACE4_DELETE but doesn't return it - expect OK
# f: Test restoring all file owner perms - expect OK
#
...
set tag "$TNAME{a}"
set ASSERTION "Test server accepts ACE4_READ_NAMED_ATTRS but doesn't return it - expect $expcode"
putmsg stdout 0 "$tag: $ASSERTION"

set sid {1 1}

set owner_allow_mask [ aclmask [ concat "ACE4_READ_NAMED_ATTRS" $OWNER_ALLOW_ACL $POSIX_READ_ACL \
		$POSIX_WRITE_ACL $POSIX_EXECUTE_ACL ] ]

set owner_deny_mask 0

# Access Mask expected back from server
set server_allow_mask [ aclmask [ concat $OWNER_ALLOW_ACL $POSIX_READ_ACL \
		$POSIX_WRITE_ACL $POSIX_EXECUTE_ACL ] ]
...
#
set mod_acl_list [lreplace $acl_list 0 0 "0 0 $owner_allow_mask OWNER\@"]
set mod_acl_list [lreplace $acl_list 1 1 "1 0 $owner_deny_mask OWNER\@"]
putmsg stderr 1 "$tag: modified ACL : $mod_acl_list"

#
# Generate the ACL list we expect to see back from the server
set srv_acl_list [lreplace $acl_list 0 0 "0 0 $server_allow_mask OWNER\@"]
set srv_acl_list [lreplace $acl_list 1 1 "1 0 $owner_deny_mask OWNER\@"]
putmsg stderr 1 "$tag: server ACL : $srv_acl_list"
...
if { $status == "OK" } {
	...
	if { [compare_acl_lists $new_acl_list $srv_acl_list] != 0} {
        	putmsg stderr 0 \
...
set tag "$TNAME{b}"
set ASSERTION "Test restoring file owner perms - expect $expcode"
putmsg stdout 0 "$tag: $ASSERTION"
...
set tag "$TNAME{c}"
set ASSERTION "Test server accepts ACE4_WRITE_NAMED_ATTRS but doesn't return it - expect $expcode"
putmsg stdout 0 "$tag: $ASSERTION"

set sid {1 1}

set owner_allow_mask [ aclmask [ concat "ACE4_WRITE_NAMED_ATTRS" $OWNER_ALLOW_ACL $POSIX_READ_ACL \
		$POSIX_WRITE_ACL $POSIX_EXECUTE_ACL ] ]
...
set tag "$TNAME{e}"
set ASSERTION "Test server accepts ACE4_DELETE but doesn't return it - expect $expcode"
putmsg stdout 0 "$tag: $ASSERTION"

set sid {1 1}

set owner_allow_mask [ aclmask [ concat "ACE4_DELETE" $OWNER_ALLOW_ACL $POSIX_READ_ACL \
		$POSIX_WRITE_ACL $POSIX_EXECUTE_ACL ] ]
...
# ------------------------------------------------------------------------
# f: Test restoring file owner perms - expect OK
