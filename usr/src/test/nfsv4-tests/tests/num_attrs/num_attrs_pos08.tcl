...
#
# Test NFSv4: 
# {a}:Test get attr FATTR4_LEASE_TIME of a filesystem object, expect OK
#
...
set ASSERTION "Test get attr FATTR4_LEASE_TIME of a filesystem, expect $expcode"
...
set attrval [ extract_attr [lindex [lindex $res 3] 2] "lease_time" ]

if { ![string equal $cont "false"] } {
	if {[string is integer $attrval]} {
		if {$attrval >= 0} {
			prn_attrs [lindex [lindex $res 3] 2]
			putmsg stdout 0 "\t Test PASS"
		}
	} else {
		putmsg stderr 0 "\t Test FAIL: FATTR4_LEASE_TIME attr is invalid"
	}
}
puts ""
