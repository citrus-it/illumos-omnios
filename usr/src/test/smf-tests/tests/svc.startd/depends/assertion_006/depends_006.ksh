...
#
# ASSERTION: depends_006
# DESCRIPTION:
#  A service with multiple dependencies in a "require_any" grouping
#  property is set in a service's definition. Once at least one of the 
#  dependencies are satisfied then the svc.startd will transition 
#  the service into the online state.
#  Services: a, b, c; a depends on b and c in a require_any relationship
#
...
readonly registration_template=$DATA/service_006.xml
...
print -- "--INFO: Wait for $test_FMRI3 to come online - it should"
service_wait_state $test_FMRI3 online
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI3 didn't come online"
	print -- "  it is in the '$(svcs -H -o STATE $test_FMRI3)' state"
	exit $STF_FAIL
fi

print -- "--INFO: Disabling service $test_FMRI1"
svcadm disable $test_FMRI1
if [ $? -ne 0 ]; then
        print -- "--DIAG: $assertion: Service $test_FMRI1 did not disable"
        exit $STF_FAIL
fi

print -- "--INFO: Waiting for $test_FMRI1 to go disabled"
service_wait_state $test_FMRI1 disabled
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI1 did not goto disabled"
	exit $STF_FAIL
fi

print -- "--INFO: Verifying that $test_FMRI3 goes offline"
...
print -- "--INFO: Disabling $test_FMRI3"
svcadm disable $test_FMRI3
if [ $? -ne 0 ]; then
        print -- "--DIAG: $assertion: Service $test_FMRI3 did not disable"
        exit $STF_FAIL
fi

print -- "--INFO: Waiting for $test_FMRI3 to go disabled"
service_wait_state $test_FMRI3 disabled
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI3 did not goto disabled"
	print -- "  it's in: '$(svcs -H -o STATE $test_FMRI3)'"
	exit $STF_FAIL
fi

print -- "--INFO: Enabling $test_FMRI3"
svcadm enable $test_FMRI3
if [ $? -ne 0 ]; then
        print -- "--DIAG: $assertion: Service $test_FMRI3 did not enable"
        exit $STF_FAIL
fi

print -- "--INFO: Waiting for $test_FMRI3 to stay offline"
service_wait_state $test_FMRI3 offline
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI3 did not goto offline"
	print -- "  it is in the '$(svcs -H -o STATE $test_FMRI3)' state"
	exit $STF_FAIL
fi
