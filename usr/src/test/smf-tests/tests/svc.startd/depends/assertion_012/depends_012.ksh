...
#  fail then the service will be transitioned into the offline state.
#  Services: a, b, c; c specifies exclude_all of (a, b)
#
...
function disable_wait {
	...
	echo "--INFO: Waiting for $service to enter disabled"
	service_wait_state $service disabled
	if [ $? -ne 0 ]; then
		echo "--DIAG: $assertion: Service $service did not get disabled
	Current state: '$(svcprop -p restarter/state $service)'"
		exit $STF_FAIL
	fi
}
...
function wait_online {
	...
	if [ $? -ne 0 ]; then
		echo "--DIAG: $assertion: Service $service did not go online
	Current state: '$(svcprop -p restarter/state $service)'"
		exit $STF_FAIL
	fi
}
...
function enable_wait {
	typeset service="$1"
	echo "--INFO: Enable service $service"
...
function wait_offline {
	typeset service="$1"
	echo "--INFO: Wait for $service to go offline"
	service_wait_state $service offline
	if [ $? -ne 0 ]; then
		echo "--DIAG: $assertion: Service $service didn't go offline
	Current state: '$(svcprop -p restarter/state $service)'"
		exit $STF_FAIL
	fi
}
...
readonly registration_template=$DATA/service_012.xml
...
wait_online $test_FMRI3

enable_wait $test_FMRI1

wait_offline $test_FMRI3
