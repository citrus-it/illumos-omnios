...
#
# ASSERTION: depends_052
# DESCRIPTION:
#  A service with a single dependency in a "require_all" grouping
#  property is specified. The dependency is satisfied.
#  svc.startd will transition the service into the online state.
#  Services: a, b; b depends on a
#  Each dependency is on a separate service, and is listed as a
...
readonly registration_template=$DATA/service_052.xml
...
print -- "--INFO: Cleanup any old $test_service1, $test_service2 state"
service_cleanup $test_service1
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: cleanup of $test_service1 instance failed"
	exit $STF_UNRESOLVED
fi
service_cleanup $test_service2
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: cleanup of $test_service2 instance failed"
	exit $STF_UNRESOLVED
fi
...
print -- "--INFO: Wait for $test_FMRI2 to come online - it should not"
service_wait_state $test_FMRI2 online
if [ $? -eq 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI2 came online"
	exit $STF_FAIL
fi
