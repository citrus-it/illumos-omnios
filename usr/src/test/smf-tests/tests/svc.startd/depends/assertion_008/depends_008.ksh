...
#  Service a, b; b is specified exclude_all of a
#
...
readonly registration_template=$DATA/service_008.xml
...
print -- "--INFO: Waiting for $test_FMRI2 to come online - it should not"
...
print -- "--INFO: Disabling $test_FMRI1"
svcadm disable $test_FMRI1
if [ $? -ne 0 ]; then
        print -- "--DIAG: $assertion: Service $test_FMRI2 did not disable"
        exit $STF_FAIL
fi

print -- "--INFO: Verifying that $test_FMRI1 goes to disabled"
service_wait_state $test_FMRI1 disabled
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI1 didn't go to disabled"
	exit $STF_FAIL
fi
