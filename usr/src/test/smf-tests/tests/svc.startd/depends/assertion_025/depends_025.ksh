...
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI2 dodn't come online"
	exit $STF_FAIL
fi
...
echo "--INFO: Sending $test_FMRI1 to maintenance mode"
...
if [ $? -ne 0 ]; then
	echo "--DIAG: $assertion: Service $test_FMRI2 didn't enter "
	echo "  offline mode"
	exit $STF_FAIL
fi
