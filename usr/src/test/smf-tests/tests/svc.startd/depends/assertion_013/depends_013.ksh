...
#
# ASSERTION: depends_013
# DESCRIPTION:
#  A service with a single dependency in a "require_any" grouping
...
print -- "--INFO: Waiting for $test_FMRI1 to go to disabled"
service_wait_state $test_FMRI1 disabled
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI1 did not go to "
	print -- "  disabled"
	exit $STF_FAIL
fi

print -- "--INFO: waiting for $test_FMRI2 to go offline"
service_wait_state $test_FMRI2 offline
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: service $test_FMRI2 did not go offline"
	exit $STF_FAIL
fi
