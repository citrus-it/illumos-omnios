...
# DESCRIPTION:
#  A service may specify a file as a dependency by specifying
#  that file in the entities property.
#
...
readonly registration_template=$DATA/service_033.xml
...
print -- "--INFO: Wait for $test_FMRI1 to go to offline"
service_wait_state $test_FMRI1 offline
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI1 didn't go to offline"
	exit $STF_FAIL
fi
...
if [ $? -ne 0 ]; then
	print -- "--DIAG: could not refresh service $test_FMRI1"
	exit $STF_UNRESOLVED
fi
