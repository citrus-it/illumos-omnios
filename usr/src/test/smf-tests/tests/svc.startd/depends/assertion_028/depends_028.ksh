...
#
# ASSERTION: depends_028
# DESCRIPTION:
#  A service which is dependent upon itself should be transitioned
#  into the maintenance state immediately.
...
readonly registration_template=$DATA/service_028.xml
...
print -- "--INFO: Wait for $test_FMRI to go to maintenance"
service_wait_state $test_FMRI maintenance
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI didn't go to "
	print -- "  maintenance."
	exit $STF_FAIL
fi
