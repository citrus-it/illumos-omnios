...
#
# ASSERTION: depends_027
# DESCRIPTION:
#  If an online service, service_A, has a dependency on another service,
#  service_B, and service_B is transitioned into degraded state, then 
#  service_A shall continue to execute unchanged (i.e. degraded
#  dependencies have no effect)
#
...
readonly registration_template=$DATA/service_027.xml
...
echo "--INFO: Sending service $test_FMRI1 to degraded"
svcadm mark degraded $test_FMRI1
if [ $? -ne 0 ]; then
...
echo "--INFO: Verify degraded mode for $test_FMRI1"
service_wait_state $test_FMRI1 degraded
if [ $? -ne 0 ]; then
...
echo "--INFO: Check for offline of $test_FMRI2 - should not happen"
service_wait_state $test_FMRI2 offline
if [ $? -eq 0 ]; then
	echo "--DIAG: $assertion: service $test_FMRI2 entered"
	echo "  offline incorrectly"
	exit $STF_FAIL
fi
