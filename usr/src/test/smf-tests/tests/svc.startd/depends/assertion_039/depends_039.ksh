...
#
# ASSERTION: depends_039
# DESCRIPTION:
#  Dependencies can be removed from an offline service. If the
#  dependency that is removed is the only dependency the offline service
#  is dependent on then the service should transition to online.
#
...
print -- "--INFO: Waiting for $test_FMRI2 to go to offline"
service_wait_state $test_FMRI2 offline
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI2 did not go to offline"
	exit $STF_FAIL
fi

print -- "--INFO: Remove dependency from $test_FMRI2 -> $test_FMRI1"
service_dependency_remove $test_FMRI2 bdepa
...
print -- "--INFO: Verfify service $test_FMRI2 is online"
service_wait_state $test_FMRI2 online
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI2 isn't online"
	exit $STF_FAIL
fi

print -- "--INFO: Disable the service $test_FMRI1"
svcadm disable $test_FMRI1
if [ $? -ne 0 ]; then
	print -- "--DIAG: could not disable $test_FMRI1"
	exit $STF_FAIL
fi

print -- "--INFO: validating service $test_FMRI2 stays online"
service_wait_state $test_FMRI2 offline
if [ $? -eq 0 ]; then
	print -- "--DIAG: service $test_FMRI2 went offline"
	exit $STF_FAIL
fi
