...
#
# ASSERTION: depends_009
# DESCRIPTION:
#  A service with multiple dependencies in an "exclude_all" grouping
#  property is set in the service's definition If none of the
#  dependencies are satisfied then the service will be transitioned
#  into the online state.
#  Service a, b, c; c is specified exclude_all of (a, b)
#
...
readonly registration_template=$DATA/service_009.xml
...
echo "--INFO: Cleanup any old $test_FMRI1, $test_FMRI2, $test_FMRI3 state"
...
echo "--INFO: Wait for $test_FMRI3 to go offline"
service_wait_state $test_FMRI3 offline
if [ $? -ne 0 ]; then
	echo "--DIAG: $assertion: $test_FMRI3 did not go offline before timeout"
	echo "	State of services:"
	svcs -l $test_service\*
	exit $STF_FAIL
fi

echo "--INFO: Disabling service $test_FMRI1"
...
echo "--INFO: Waiting for $test_FMRI1 to go to disabled"
service_wait_state $test_FMRI1 disabled
if [ $? -ne 0 ]; then
	echo "--DIAG: $assertion: Service $test_FMRI1 did not get disabled
...
echo "--INFO: Disabling service $test_FMRI2"
svcadm disable $test_FMRI2
if [ $? -ne 0 ]; then
        echo "--DIAG: $assertion: Service $test_FMRI2 did not disable"
        exit $STF_FAIL
fi

echo "--INFO: Waiting for $test_FMRI2 to go to disabled"
service_wait_state $test_FMRI2 disabled
if [ $? -ne 0 ]; then
	echo "--DIAG: $assertion: Service $test_FMRI2 did not get disabled
