...
#
# ASSERTION: depends_040
# DESCRIPTION:
#  Dependencies can be removed from an offline service.  If the
#  dependency that is removed is not the only dependency the offline
#  service is dependent on then the service should remain in the
#  offline state.
#
...
print -- "--INFO: Waiting for $test_FMRI3 to go to offline"
service_wait_state $test_FMRI3 offline
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI3 did not go to offline"
	exit $STF_FAIL
fi

print -- "--INFO: Remove dependency from $test_FMRI3 -> $test_FMRI1"
service_dependency_elt_remove $test_FMRI3 cdepab svc:/$test_FMRI1
...
print -- "--INFO: Verify service $test_FMRI3 is online - it shouldn't"
service_wait_state $test_FMRI3 online
if [ $? -eq 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI3 is online"
	exit $STF_FAIL
fi
