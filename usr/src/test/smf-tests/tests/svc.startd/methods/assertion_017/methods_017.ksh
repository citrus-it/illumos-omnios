...
#
# ASSERTION: methods_017
# DESCRIPTION:
#  If a service instance in the degraded state dumps core than svc.startd 
#  should attempt to restart the service. The service has one process
#
...
registration_template=$DATA/service_017.xml
...
print -- "--INFO: Creating monitor returncode file"
echo "returncode $SVC_DEGRADED" > $monitor_errfile
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: unable to create $monitor_errfile"
	exit $STF_UNRESOLVED
fi
...
if [ $rc -ne 0 ]; then
	...
	EXPECTED: svcadm returned 0
	OBSERVED: svcadm returned $rc"
	exit $STF_FAIL
fi
...
print -- "--INFO: marking state to degraded"
svcadm mark degraded $test_FMRI
rc=$?
if [ $rc -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI didn't enter degraded
	EXPECTED: 'svcadm mark degraded $test_FMRI' returned 0
	OBSERVED: 'svcadm mark degrated $test_FMRI' returned $rc"
	exit $STF_UNRESOLVED
fi
...
if [ $? -ne 0 ]; then
	...
	EXPECTED: service state = degraded
	OBSERVED: service state = `svcprop -p restarter/state $test_FMRI`"
	exit $STF_FAIL
fi
...
print -- "--INFO: remove monitor's reason for issuing SVC_DEGRADED"
