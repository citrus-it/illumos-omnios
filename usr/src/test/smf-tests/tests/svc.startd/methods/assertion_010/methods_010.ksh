...
#
# ASSERTION: methods_010
# DESCRIPTION:
#  Upon starting a service the svc.startd invokes the service's start
#  method.
#  Should the method returns a non-zero exit code other than the
#  defined straight to maintenance exit code it is considered to have
#  failed, but not irrevocably.
#  The svc.startd will re-attempt to start the service until the
#  error threshold is reached at which point the service is
#  transitioned to the maintenance state.
#
...
registration_template=$DATA/service_010.xml
...
manifest_generate $registration_template \
	...
	START_EVENT="returncode $SVC_METHOD_OTHEREXIT" \
	> $registration_file
