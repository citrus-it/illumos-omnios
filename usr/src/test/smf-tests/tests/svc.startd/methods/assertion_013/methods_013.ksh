...
#
# ASSERTION: methods_013
# DESCRIPTION:
#  If svc.startd invokes a service's refresh method and that method
#  returns a non-zero exit value other than the defined straight to
#  maintenance return code then the service is considered to have
#  failed, but not irrevocably. The svc.startd will re-attempt to
#  refresh the service until the error threshold is reached at
#  which point the service is transitioned to the maintenance state.
#
...
registration_template=$DATA/service_013.xml
...
manifest_generate $registration_template \
	...
	REFRESH_EVENT="returncode $SVC_METHOD_OTHEREXIT" \
	> $registration_file
