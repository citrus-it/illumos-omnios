...
#
# ASSERTION: methods_025
# DESCRIPTION:
#  A service with a single process.
#  If a service instance in the degraded state dies due to receipt of
#  a fatal signal from another process then the svc.startd should attempt 
...
registration_template=$DATA/service_025.xml
...
print -- "--INFO: marking service as degraded"
svcadm mark degraded $test_FMRI
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI could not be placed"
	print -- "  in the degraded state"
	exit $STF_UNRESOLVED
fi
...
print -- "--INFO: issue a kill to the service pid"
kill $PIDS
if [ $? -ne 0 ]; then
        print "--DIAG: $assertion: could not issue kill to $PID"
        exit $STF_UNRESOLVED
fi
