...
#
# ASSERTION: methods_002
# DESCRIPTION:
#  When a service that has been registered and is running in the smf
#  framework is disabled it's stop method will be invoked and it will be
...
registration_template=$DATA/service_002.xml
...
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: unable to delete previous service state"
	exit $STF_UNRESOLVED
fi
...
if [ "$lastcalled" != "stop" ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI didn't call stop "
