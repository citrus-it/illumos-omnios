...
registration_template=$DATA/service_022.xml
...
$service_app -s $test_service -i $test_instance \
	-m trigger -r "triggerchildrv 1 0"

print -- "--INFO: Waiting for service's start method ... should not be called"
service_wait_method $test_FMRI start
if [ $? -eq 0 ]; then
	print -- "--DIAG: $assertion: issued start method for $test_FMRI"
	exit $STF_FAIL
fi

print -- "--INFO: Finishing off second process"
$service_app -s $test_service -i $test_instance \
