...
#
# ASSERTION: methods_027
# DESCRIPTION:
#  Service with a single process.
#  If a service instance in the online state dies due to a hwerr
...
registration_template=$DATA/service_027.xml
...
service_wait_method $test_FMRI memcntl 5
if [ $? -ne 0 ]; then
	print "--DIAG: could not mlock memory space of service"
	exit $STF_UNRESOLVED
fi

print -- "--INFO: issue a hardware error to the service pid"
$service_app -s $test_service -i $test_instance -l $service_log -m "trigger" \
