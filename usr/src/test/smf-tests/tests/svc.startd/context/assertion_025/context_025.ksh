...
#
# ASSERTION: context_025
# DESCRIPTION:
#  If no project attribute is given, the svc.startd will set
#  the project to :default given the user/group specified's
#  default project.
#
...
readonly registration_template=$DATA/service_025.xml
...
print -- "--INFO: Checking start method's project name"
line=`grep_logline_entry $test_service $test_instance start projectname`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find projectname line from "
	print -- " start method '${line}'"
	exit $STF_FAIL
fi
if [ "${line}" != "$ctx_project" ]; then
	print -- "--DIAG: Did not have working name '$ctx_project'."
