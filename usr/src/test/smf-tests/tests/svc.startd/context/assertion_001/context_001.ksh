...
#
# ASSERTION: context_001
# DESCRIPTION:
#  svc.startd will start a method using the privilege set specified
#  by the privileges attribute of the method_credential element.
#
...
readonly registration_template=$DATA/service_001.xml
...
print -- "--INFO: Checking start methods credential set file_dac_write"
line=`grep_logline_entry $test_service $test_instance start privileges`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find privileges line from "
	print -- " start method '${line}'"
	exit $STF_FAIL
fi
if [ "${line%file_dac_write*}" = "${line}" ]; then
	print -- "--DIAG: Did not have permission file_dac_write in set $line"
	exit $STF_FAIL
fi
