...
#
# ASSERTION: context_044
# DESCRIPTION:
#  Attempting to start a service with an inaccessible working_directory
...
readonly registration_template=$DATA/service_044.xml
...
echo "--INFO: enforce a+rx for all directories down to $RUNDIR"
typeset permdir=$RUNDIR
while :; do
	[ $permdir = "/" ] && break
	chmod a+rx $permdir
	if [ $? -ne 0 ]; then
		echo "--DIAG: $assertion: could not chmod $permdir a+rx"
		exit $STF_UNRESOLVED
	fi
...
TESTDIR="$RUNDIR/test_dir"
if [ ! -d "$TESTDIR" ]; then
	mkdir -p "$TESTDIR"
	if [ $? -ne 0 ]; then
		echo "--DIAG: $assertion: could not create $TESTDIR"
		exit $STF_UNRESOLVED
	fi
fi
...
chmod a-rwx "$TESTDIR"
if [ $? -ne 0 ]; then
	echo "--DIAG: $assertion: could not set $TESTDIR to inacecssible"
	exit $STF_UNRESOLVED
fi
...
print -- "--INFO: Wait for $test_FMRI to goto maintenance"
service_wait_state $test_FMRI maintenance
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI did not go"
	print -- "  to maintenance. It was in '$(svcs -H -o STATE $test_FMRI)'"
	exit $STF_FAIL
fi
