...
#
# ASSERTION: context_016
# DESCRIPTION:
#  Attempting to start a service with an invalid profile set in the
#  method_profile attribute will result in an error.  svc.startd
#  will re-attempt to start the service until the error threshold
#  is reached and, at that point, the service will be placed into
#  maintenance state.
#
...
readonly registration_template=$DATA/service_016.xml
...
manifest_generate $registration_template \
	...
	LOGFILE=$service_log \
	TEST_PROFILENAME="This Profile Certainly Does Not Exist Here" \
...
print -- "--INFO: Wait for $test_FMRI to enter maintenance"
service_wait_state $test_FMRI maintenance
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: Service $test_FMRI did not go to"
	print -- "  maintenance. It was in '$(svcs -H -o STATE $test_FMRI)'."
	exit $STF_FAIL
fi
