...
#
# ASSERTION: context_004
# DESCRIPTION:
#  svc.startd will start a method using the group credentials specified
#  by the group attribute in numeric form of the method_credential element
#
...
readonly registration_template=$DATA/service_004.xml
...
print -- "--INFO: Checking start method's groupid is $CTX_GID"
line=`grep_logline_entry $test_service $test_instance start groupid`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find groupid line from "
	print -- " start method '${line}'"
	exit $STF_FAIL
fi
if [ "$line" != "$CTX_GID" ]; then
	print -- "--DIAG: $assertion: groupid of execution $line"
	print -- "  wasn't $CTX_GID"
	exit $STF_FAIL
fi
