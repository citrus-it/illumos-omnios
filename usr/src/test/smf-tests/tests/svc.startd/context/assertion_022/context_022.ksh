...
#
# ASSERTION: context_022
# DESCRIPTION:
#  :default can be used as a token to indicate the use of the project
#  identified by getdefaultproj(3PROJECT) for the user specified
#  by the method_credential.
#
...
readonly registration_template=$DATA/service_022.xml
...
proj=$(projects -d $ctx_user | awk '{print $1}')

print -- "--INFO: Checking start methods working project name"
line=`grep_logline_entry $test_service $test_instance start projectname`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find project line from"
	print -- " start method"
	exit $STF_FAIL
fi
if [ "${line}" != "$proj" ]; then
	print -- "--DIAG: Did not have '$proj' as project."
