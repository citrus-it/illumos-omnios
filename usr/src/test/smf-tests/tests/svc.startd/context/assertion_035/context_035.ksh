...
#
# ASSERTION: context_035
# DESCRIPTION:
#  Supplemental groups specified for a service method cause that
#  service's method to be executed with the supplemental group ids
#  set to those in the set.
#
...
print -- "--INFO: acquiring the name of 5 groups" 
while [ $ngroups -lt 5 ]; do
...
print -- "--INFO: Checking start methods suppgroupnames are '$SUPPGROUPS'"
line=`grep_logline_entry $test_service $test_instance start suppgroupnames`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find suppgroupnames line from "
	print -- " start method '${line}'"
	exit $STF_FAIL
fi
if [ "$line" != "$SUPPGROUPS" ]; then
	print -- "--DIAG: $assertion: groups of execution $line wasn't "
	print -- "  $SUPPGROUPS"
	exit $STF_FAIL
fi
