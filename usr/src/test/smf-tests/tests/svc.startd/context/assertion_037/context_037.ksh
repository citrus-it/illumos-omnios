...
#
# ASSERTION: context_037
# DESCRIPTION:
#  svc.startd will not start a method using the limit privilege set
#  specified by the limit_privileges attribute of the
#  method_credential element if the set specified does not conform
#  to a privilege set, but will instead transition it to maintenance
#  mode.
#
...
# construct a privileges string that is invalid
typeset limitprivs="winged_flying_lemurs"
...
echo "--INFO: Make sure the $RUNDIR is world r/w/x"
chmod a+rwx $RUNDIR
if [ $? -ne 0 ]; then
	echo "--DIAG: $assertion: could not make $RUNDIR mode 777"
	exit $STF_UNRESOLVED
fi
...
if [ $? -ne 0 ]; then
	...
	echo "  it is in '$(svcs -H -o STATE $test_FMRI)' mode."
	exit $STF_FAIL
fi
