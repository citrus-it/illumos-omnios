...
#
# ASSERTION: context_021
# DESCRIPTION:
#  ":home" can be used as a token to indicate the use of the working
#  directory of the user specified in the method_credential.
#
...
readonly registration_template=$DATA/service_021.xml
...
TESTDIR=`getent passwd $ctx_user | cut -d: -f6`

print -- "--INFO: Checking start methods working directory"
line=`grep_logline_entry $test_service $test_instance start mycwd`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find mycwd line from "
	print -- " start method '${line}'"
	exit $STF_FAIL
fi
if [ "${line}" != "$TESTDIR" ]; then
	print -- "--DIAG: Did not have workign directory $TESTDIR."
