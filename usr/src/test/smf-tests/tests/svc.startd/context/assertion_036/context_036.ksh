...
#
# ASSERTION: context_036
# DESCRIPTION:
#  svc.startd will start a method using the limit privilege set specified
#  by the limit_privileges attribute of the method_credential element.
#
...
# remove all dtrace privileges
typeset limitprivs=all
typeset priv=
for priv in $(ppriv -l | grep dtrace); do
	limitprivs=$limitprivs,-$priv
done
...
print -- "--INFO: Make sure the $RUNDIR is world r/w/x"
chmod a+rwx $RUNDIR
if [ $? -ne 0 ]; then
	print -- "--DIAG: $assertion: could not make $RUNDIR mode 777"
	exit $STF_UNRESOLVED
fi
...
print -- "--INFO: Checking start methods credential removed dtrace limit set"
line=`grep_logline_entry $test_service $test_instance start limit_privileges`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find limit privileges line from "
	print -- " start method '${line}'"
	exit $STF_FAIL
fi

if [ "${line%dtrace*}" != "${line}" ]; then
	print -- "--DIAG: privileges '$line' had 'dtrace' entries"
	exit $STF_FAIL
fi
