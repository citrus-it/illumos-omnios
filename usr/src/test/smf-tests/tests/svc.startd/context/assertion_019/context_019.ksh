...
#
# ASSERTION: context_019
# DESCRIPTION:
#  svc.startd will start a method in the project specified in
...
readonly registration_template=$DATA/service_019.xml
...
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find projname line from "
	print -- " start method"
	exit $STF_FAIL
fi
if [ "${line}" != "$ctx_project" ]; then
	print -- "--DIAG: Did not have project named $ctx_project."
