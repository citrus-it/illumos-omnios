...
#
# ASSERTION: context_041
# DESCRIPTION:
#  svc.startd will start a method in the projectid specified in
#  the project attribute of the method_context.
#
...
readonly context_projid=$(getent project $ctx_project | cut -d: -f 2,2)
...
line=`grep_logline_entry $test_service $test_instance start projectid`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find projectid line from "
	print -- " start method"
	exit $STF_FAIL
fi
if [ "${line}" != "$context_projid" ]; then
	print -- "--DIAG: Did not have project id $context_projid."
