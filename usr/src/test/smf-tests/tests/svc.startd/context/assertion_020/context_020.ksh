...
#
# ASSERTION: context_020
# DESCRIPTION:
#  svc.startd will start a method from the resource pool specified in
#  the resource_pool attribute of the method_context.
#
...
readonly registration_template=$DATA/service_020.xml
...
print -- "--INFO: Checking start method's resource pool"
line=`grep_logline_entry $test_service $test_instance start poolinfo`
if [ $? -ne 0 ]; then
	print -- "--DIAG: Could not find poolinfo line from "
	print -- " start method"
	exit $STF_FAIL
fi
if [ "${line}" != "$ctx_resourcepool" ]; then
	print -- "--DIAG: Did not have poolinfo named $ctx_resourcepool."
