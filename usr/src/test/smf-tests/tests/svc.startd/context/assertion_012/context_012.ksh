...
#
# ASSERTION: context_012
# DESCRIPTION:
#  If no privileges attribute is set, then svc.startd will start
#  a method using the privilege set of the default user (uid=0).
#
...
privilegeset=`ppriv -v $$ | sed -n 's!.*E: !!p'`

print -- "--INFO: Checking start methods privilege set"
...
if [ "$line" != "$privilegeset" ]; then
	print -- "--DIAG: Did not have privilege set '$privilegeset'"
	print -- " instead there was privilege set: '$line'"
	exit $STF_FAIL
fi
