...
#
# ASSERTION: svcadm_enable_003
#
# DESCRIPTION:
#	Calling 'svcadm enable FMRI' where FMRI is a service instance that is
#	disabled state will result in the service being started and
#	transitioned into the enabled state.  The exit status shall be 0.
# STRATEGY:
#	- Check for test setup.
#	- Configure a service 'foo$$' using svccfg
#	- Disable it using svcadm
#	- Make sure state is disabled.
#	- Now enable the service using svcadm enable
#	- Verify that exit status of above step is 0.
#	- Also verify that state is transitioned to online.
#
# COMMANDS: svcadm(1)
#
...
# Define Variables
readonly assertion=svcadm_enable_003
readonly svccfg_add_script=/var/tmp/svcadm_enable_003.$$.config
...
service_wait_state $service_test:$instance_test disabled
if [ $? -ne 0 ]; then
        echo "--DIAG: [$assertion]
