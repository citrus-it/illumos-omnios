...
#
# ASSERTION: svcadm_enable_008
#
# DESCRIPTION:
# 	Calling 'svcadm enable FMRI' over a service which
# 	is currently online with running processes, should
# 	succeed, without disturbing the service's state and running
# 	process(es).
# STRATEGY:
#	- locate for template.xml in $STF_SUITE/tests/svcadm/
#	- Create a simple test_process that sleeps for may be 10 secs.
#	- Generate say 'svcadm_enable_008.$$.xml' using template.xml 
...
# Define Variables
readonly assertion=svcadm_enable_008
readonly ME=$(whence -p ${0})
readonly MYLOC=$(dirname ${ME})
readonly registration_template=${STF_SUITE}/tests/svcadm/enable/template.xml
readonly registration_file=/var/tmp/svcadm_enable_008.$$.xml
readonly test_service="enable_008$$"
readonly test_instance="enable_008$$"
readonly test_process=enable_008.$$
...
echo "--INFO: [${assertion}]
	Verify state remains unchanged from online"

state=`svcprop -p restarter/state svc:/$test_service:$test_instance 2>/dev/null`
if [[ $? -ne 0 || "$state" != "online" ]]; then
	RESULT=$(update_result $STF_FAIL $RESULT)
	echo "--DIAG: [$assertion]
	svcs -Ho STATE $test_service
	EXPECTED: ret =0; state=online
	ACTUAL: ret = $ret; state= online"
	print_result $RESULT
	exit $RESULT
fi
