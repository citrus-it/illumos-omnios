...
#
# ASSERTION: svcadm_enable_007
#
# DESCRIPTION:
#	Calling 'svcadm -v enable FMRI' over an instance that is
#	currently in disabled state should transition the state to
#	online and start any process associated with it and also
#	should display "fmri displayed" message in output.
# STRATEGY:
#	- locate for template.xml in $STF_SUITE/tests/svcadm/
#	- Create a simple test_process that sleeps for may be 10 secs.
#	- Generate say 'svcadm_enable_007.$$.xml' using template.xml 
...
#	- Now attempt to enable the FMRI using svcadm -v <fmri> enable
...
# Define Variables
readonly assertion=svcadm_enable_007
readonly ME=$(whence -p ${0})
readonly MYLOC=$(dirname ${ME})
readonly registration_template=${STF_SUITE}/tests/svcadm/enable/template.xml
readonly registration_file=/var/tmp/svcadm_enable_007.$$.xml
readonly test_service="enable_007$$"
readonly test_instance="enable_007$$"
readonly test_process=enable_007.$$
...
echo "--INFO: [${assertion}]
	enable <$fmri> using svcadm -v <$fmri> enable"

output=`svcadm -v enable svc:/$test_service:$test_instance 2>/dev/null`
ret=$?
if [[ $ret -ne 0 || "$output" != "$fmri enabled." ]]; then
	RESULT=$(update_result $STF_FAIL $RESULT)
	echo "--DIAG: [$assertion]
		svcadm disable svc:/$test_service:$test_instance fails
	EXPECTED: exit = ret 0, output=svc:/$test_service:$test_instance enabled
	ACTUAL: exit ret = $ret, output=$fmri enabled."
	print_result $RESULT
	exit $RESULT
fi
