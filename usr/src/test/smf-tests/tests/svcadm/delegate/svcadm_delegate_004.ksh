...
#
# ASSERTION: svcadm_delegate_004
#
# DESCRIPTION:
#	Calling 'svcadm delegate <invalid restarter> <online service>' where 
...
#	- Call svcadm delegate <invalid restarter> <online service>
#	- Verify above step exits 1.
#
# COMMANDS: svcadm(1)
#
...
# Define Variables
readonly assertion=svcadm_delegate_004
readonly svccfg_add_script=/var/tmp/svcadm_delegate_004.$$.config
readonly service_test=foo$$
readonly instance_test=instance$$
readonly invalid_restarter="svc:/invalid_fmri"
...
echo "--INFO: [${assertion}]
	svcadm delegate <invalid-restarter> <valid-fmri>"

/usr/sbin/svcadm delegate $invalid_restarter $valid_fmri >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
	RESULT=$(update_result $STF_FAIL $RESULT)
	echo "--DIAG: [$assertion]
	svcadm delegate $invalid_restarter $valid_fmri should fail and exit 1
	But it returned : $ret"
	print_result $RESULT
	exit $RESULT
fi
