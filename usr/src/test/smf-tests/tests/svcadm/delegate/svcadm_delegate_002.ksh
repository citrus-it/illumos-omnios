...
# Define Variables
readonly assertion=svcadm_delegate_002
...
svcadm -v delegate $invalid_fmri $invalid_fmri >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
...
echo "--INFO: [${assertion}]
	Call svcadm -v delegate $valid_fmri $invalid_fmri"

svcadm -v delegate $valid_fmri $invalid_fmri >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
    RESULT=$(update_result $STF_FAIL $RESULT)
    echo "--DIAG: [$assertion]
...
echo "--INFO: [${assertion}]
	Call svcadm -v delegate $invalid_fmri $valid_fmri"

svcadm -v delegate $invalid_fmri $valid_fmri >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
    RESULT=$(update_result $STF_FAIL $RESULT)
    echo "--DIAG: [$assertion]
...
echo "--INFO: [${assertion}]
	Call svcadm -v delegate $invalid_name $valid_name"

svcadm -v delegate $invalid_name $valid_name >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
    RESULT=$(update_result $STF_FAIL $RESULT)
    echo "--DIAG: [$assertion]
...
echo "--INFO: [${assertion}]
	Call svcadm -v delegate $valid_name $invalid_name"

svcadm -v delegate $valid_name $invalid_name >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
    RESULT=$(update_result $STF_FAIL $RESULT)
    echo "--DIAG: [$assertion]
...
echo "--INFO: [${assertion}]
	Call svcadm -v delegate $invalid_name $invalid_name"

svcadm -v delegate $invalid_name $invalid_name >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
    RESULT=$(update_result $STF_FAIL $RESULT)
    echo "--DIAG: [$assertion]
