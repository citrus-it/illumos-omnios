...
#
# ASSERTION: svcadm_disable_003
#
# DESCRIPTION:
#	Calling 'svcadm disable FMRI' where FMRI is a service instance that is
#	in the enabled state will result in the service being stopped and
#	being transitioned into disabled state.  The exit status shall be 0
# STRATEGY:
#	- locate for template.xml in $STF_SUITE/tests/svcadm/
#	- Create a simple test_process that sleeps for may be 10 secs.
#	- Generate say 'svcadm_disable_003.$$.xml' using template.xml 
...
#	- Make sure svcadm disable <service> is successful with exit 0.
...
# Define Variables
readonly assertion=svcadm_disable_003
readonly ME=$(whence -p ${0})
readonly MYLOC=$(dirname ${ME})
readonly registration_template=${STF_SUITE}/tests/svcadm/disable/template.xml
readonly registration_file=/var/tmp/svcadm_disable_003.$$.xml
readonly test_service="disable_003$$"
readonly test_instance="disable_003$$"
readonly test_process=disable_003.$$
pid=""
...
echo "--INFO: [${assertion}]
	Wait until service STATE gets updated"
