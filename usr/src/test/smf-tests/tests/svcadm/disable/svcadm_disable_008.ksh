...
#
# ASSERTION: svcadm_disable_008
#
# DESCRIPTION:
#	svcadm disable <FMRI list> should succeed
# STRATEGY:
...
#	- svcadm disable <fmrilist> and make sure it exits 0.
...
# Define Variables
readonly assertion=svcadm_disable_008
readonly svccfg_add_script1=/var/tmp/svcadm_disable_008.$$.config1
readonly svccfg_add_script2=/var/tmp/svcadm_disable_008.$$.config2
readonly service_test1=foo$$1
readonly service_test2=foo$$2
readonly instance_test=instance$$
readonly inst1_fmri=svc:/$service_test1:$instance_test
readonly inst2_fmri=svc:/$service_test2:$instance_test
...
service_wait_state $service_test1:$instance_test online
if [ $? -ne 0 ]; then
	echo "--DIAG: [$assertion]
...
service_wait_state $service_test2:$instance_test online
if [ $? -ne 0 ]; then
	echo "--DIAG: [$assertion]
...
/usr/sbin/svcadm disable $inst1_fmri $inst2_fmri >/dev/null 2>&1
ret=$?
if [ $ret -ne 0 ]; then
	RESULT=$(update_result $STF_FAIL $RESULT)
	echo "--DIAG: [$assertion]
	svcadm disable $inst1_fmri $inst2_fmri failed.
	EXPECTED: ret = 0
	ACTUAL: ret = $ret"
	print_result $RESULT
	exit $RESULT
