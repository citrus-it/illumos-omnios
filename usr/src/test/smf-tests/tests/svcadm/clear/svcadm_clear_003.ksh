...
#
# ASSERTION: svcadm_clear_003
#
# DESCRIPTION:
#	Calling 'svcadm clear <service>, where service
...
#	- Now attempt to put them in clear mode.
#	- Make sure svcadm for clear <service> fails  with
...
# Define Variables
readonly assertion=svcadm_clear_003
readonly svccfg_add_script=/var/tmp/svcadm_clear_003.$$.config
...
echo "--INFO: [${assertion}]
	Now attempt to clear "

# Invoke clear on the service
/usr/sbin/svcadm clear $service_test >/dev/null 2>&1
if [ $? -ne 1 ]; then
	RESULT=$(update_result $STF_FAIL $RESULT)
	echo "--DIAG: [$assertion]
	svcadm clear $service_test should fail"
	print_result $RESULT
	exit $RESULT
fi
