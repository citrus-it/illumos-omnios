...
#
# ASSERTION: svcadm_clear_007
#
# DESCRIPTION:
#	svcadm clear <without any arguments> should fail with exit 2.
#	svcadm -v clear <without any arguments> should fail with exit 2.
# STRATEGY:
#
# COMMANDS: svcadm(1)
#
...
# Define Variables
readonly assertion=svcadm_clear_007
...
echo "--INFO: [${assertion}]
	Call svcadm clear <no arguments>"

svcadm clear >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
    RESULT=$(update_result $STF_FAIL $RESULT)
    echo "--DIAG: [$assertion]
	EXPECTED: svcadm clear <no arguments> should exit 2
	ACTUAL: svcadm clear <no arguments> exited with $ret"
    print_result $RESULT
    exit $RESULT
fi

echo "--INFO: [${assertion}]
	Call svcadm -v clear <no arguments>"

svcadm -v clear >/dev/null 2>&1
ret=$?
if [ $ret -ne 2 ]; then
    RESULT=$(update_result $STF_FAIL $RESULT)
    echo "--DIAG: [$assertion]
	EXPECTED: svcadm -v clear <no arguments> should exit 2
	ACTUAL: svcadm -v clear <no arguments> exited with $ret"
    print_result $RESULT
    exit $RESULT
fi
