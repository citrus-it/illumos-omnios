...
#
# ASSERTION: svcadm_clear_001
#
# DESCRIPTION:
#	svcadm <clear> FMRI, where FMRI is a service instance which
#	is currently in online STATE should fail with exit 1
#
# STRATEGY:
#	- Check for test setup.
#	- Configure a service 'foo$$' using svccfg
#	- Enable it using svcadm
#	- Call svcadm <clear> <FMRI>
#	- Verify that above step fails with exit 1
#	- Also make sure STATE doesn't change from online.
#
# COMMANDS: svcadm(1)
#
...
# Define Variables
readonly assertion=svcadm_clear_001
readonly svccfg_add_script=/var/tmp/svcadm_clear_001.$$.config
...
/usr/sbin/svcadm clear $fmri >/dev/null 2>&1
if [ $? -ne 1 ]; then
	echo "--DIAG: [$assertion]
	svcadm clear $service_test should fail with exit 1"
	print_result $RESULT
	exit $RESULT
fi
