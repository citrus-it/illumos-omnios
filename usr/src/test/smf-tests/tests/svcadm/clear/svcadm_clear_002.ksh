...
# Define Variables
readonly assertion=svcadm_clear_002
readonly svccfg_add_script=/var/tmp/svcadm_clear_002.$$.config
...
if [ $? -ne 1 ]; then
	echo "--DIAG: [$assertion]
	svcadm -v clear $service_test should fail with exit 1"
	print_result $RESULT
	exit $RESULT
fi
