...
#
# ASSERTION: svcadm_degraded_002
#
# DESCRIPTION:
#	Calling 'svcadm -v mark degraded FMRI' where FMRI is a service 
...
#	- Verify that output is null
#	- Also verify it prints the message "Action degraded set for 
#		$FMRI"
...
# Define Variables
readonly assertion=svcadm_degraded_002
readonly ME=$(whence -p ${0})
readonly MYLOC=$(dirname ${ME})
readonly test_service="degraded_002$$"
readonly test_instance="degraded_002$$"
readonly test_fmri="svc:/$test_service:$test_instance"
readonly svccfg_add_script=/var/tmp/degraded_002$$.cfg
...
if [ $ret -ne 0 ]; then
        echo "--DIAG: [$assertion]
		svcadm enable $test_service fails
        EXPECTED: ret = 0
        OBSERVED: ret = $ret"
        print_result $RESULT
        exit $RESULT
...
echo "--INFO: [${assertion}]
        Verify that service state is 'disabled'"
...
output=`svcadm -v mark degraded $test_fmri 2>/dev/null`
ret=$?
if [[ $ret -ne 1 || ! -z $output ]]; then
        RESULT=$(update_result $STF_FAIL $RESULT)
        echo "--DIAG: [$assertion]
		<svcadm mark degraded $test_fmri> should fail
        EXPECTED: ret = 1; output = <empty>
