...
#
# ASSERTION: svcadm_restart_007
#
# DESCRIPTION:
#	Calling 'svcadm restart <FMRI1> <FMRI2>' where FMRI1 and FMRI2 is a 
#	service instance that is in the online state will result in the 
#	service being stopped and restarted.  The exit status shall be 0.
#
# STRATEGY:
...
# Define Variables
readonly assertion=svcadm_restart_007
readonly ME=$(whence -p ${0})
readonly MYLOC=$(dirname ${ME})
readonly registration_template=${STF_SUITE}/tests/svcadm/restart/template.xml
readonly registration_file1=/var/tmp/svcadm_restart_007.$$1.xml
readonly registration_file2=/var/tmp/svcadm_restart_007.$$2.xml
...
echo "--INFO: [$assertion]
        Verify if required manifest template exists"
...
# Create process scripts for test_service1
echo "--INFO: [$assertion]
	(for $test_service1):
		$start_process1
		$stop_process1"
...
# Generate the test service manifest for test service1
echo "--INFO: [$assertion]
        Generate .xml required for $test_service1"
...
# Generate the test service manifest for test service2
echo "--INFO: [$assertion]
        Generate .xml required for $test_service2"
...
# Verify that both registration templates were created properly
echo "--INFO: [$assertion]
        Verify that registration templates exist and size > 0 bytes"
...
# Wait until both services have transitioned to online state
...
if [ -f $start_file1 ]; then
	echo "--DIAG: [$assertion]
	$start_file1 should get removed, it is still there"
	exit $RESULT
fi
...
if [ -f $start_file2 ]; then
	echo "--DIAG: [$assertion]
	$start_file2 should get removed, it is still there"
	exit $RESULT
fi

# Restart the test services.
echo "--INFO: [${assertion}]
        restart <$fmri1> <$fmri2> using svcadm"

svcadm restart $fmri1 $fmri2 >/dev/null 2>&1
ret=$?
if [ $ret -ne 0 ]; then
        RESULT=$(update_result $STF_FAIL $RESULT)
        echo "--DIAG: [$assertion]
		svcadm restart $fmri1 $fmri2 fails
...
if [ $? -ne 0 ]; then
	RESULT=$(update_result $STF_FAIL $RESULT)
	echo "--DIAG: [$assertion]
	$stop_process2 not invoked"
	exit $RESULT
fi

# Verify that the start processes were loaded
echo "--INFO: [$assertion]
        Wait until $start_process1 and $start_process2 have been triggered"
