...
#	- Enable the instance using svcadm enable
...
# Define Variables
readonly assertion=svcadm_restart_005
readonly ME=$(whence -p ${0})
readonly MYLOC=$(dirname ${ME})
readonly test_service="smftest_svcadm"
readonly test_instance="$assertion"
readonly test_fmri="$test_service:$test_instance"
readonly svccfg_add_script=/var/tmp/restart_005$$.cfg
...
echo "--INFO: [${assertion}]
        Disable svc:/$test_service:$test_instance using svcadm disable"
...
svcadm restart $test_fmri >/dev/null 2>&1
ret=$?
if [ $ret -ne 0 ]; then
        RESULT=$(update_result $STF_FAIL $RESULT)
        echo "--DIAG: [$assertion]
		svcadm restart $test_fmri fails
        EXPECTED: output = ret 0
        ACTUAL: output ret = $ret"
        print_result $RESULT
        exit $RESULT
