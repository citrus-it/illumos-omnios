...
#
# ASSERTION: svcadm_refresh_003
#
# DESCRIPTION:
#	Calling 'svcadm refresh FMRI' where FMRI is a service instance that is
#	in the disabled state will have no effect on the service.
...
function cleanup {
	service_cleanup $refresh_test_service $svccfg_add_script
	print_result $RESULT
}

# Define Variables
readonly assertion=svcadm_refresh_003
readonly ME=$(whence -p ${0})
readonly MYLOC=$(dirname ${ME})
readonly refresh_test_service="refresh_003$$"
readonly refresh_test_instance="refresh_003$$"
readonly refresh_test_fmri="svc:/$refresh_test_service:$refresh_test_instance"
readonly svccfg_add_script=/var/tmp/refresh_003$$.cfg
...
echo "--INFO: [${assertion}]
        Enable $refresh_test_fmri using svcadm enable"
...
# Wait until the start method has been executed
echo "--INFO: [${assertion}]
...
#
echo "--INFO: [$assertion]
	Invoke \"svcadm refresh $refresh_test_fmri\""
