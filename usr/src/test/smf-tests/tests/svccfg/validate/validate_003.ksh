...
#
# ASSERTION: svccfg_validate_004
#
# DESCRIPTION:
#	Calling the 'validate' subcommand without a file name will return
...
svccfg validate > $OUTFILE 2>$ERRFILE
ret=$?
[[ $ret -ne 1 ]] &&  {
	echo "--DIAG: [${assertion}]
...
if ! egrep -s "$NO_INSTANCE_SELECTED_ERRMSG" $ERRFILE
then
	echo "--DIAG: [${assertion}]
	Expected error message \"$NO_INSTANCE_SELECTED_ERRMSG\"
	but got \"$(cat $ERRFILE)\""
