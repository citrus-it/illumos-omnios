...
#
# ASSERTION: svccfg_validate_003
#
# DESCRIPTION:
#	Calling the "validate file" subcommand with an invalid file
...
#
# STRATEGY:
#	Test the validate subcommand using each of the types of files
#	listed in the assertion above.  We don't check the error
#	output generated by validate because it is likely to change
...
# Test #1: validate an empty file

typeset -i TEST_RESULT=$STF_PASS

echo "--INFO: Validate an empty file"
...
svccfg validate ${TEST_FILE} > $OUTFILE 2>$ERRFILE
ret=$?
[[ $ret -ne 1 ]] &&  {
	echo "--DIAG: [${assertion}, test 1]
...
# Test #2: validate a regular text file

typeset -i TEST_RESULT=$STF_PASS

echo "--INFO: Validate a regular text file"
...
# Test #3: validate a binary file

typeset -i TEST_RESULT=$STF_PASS

echo "--INFO: Validate a binary file"
...
# Test #4: validate a non-existent file

typeset -i TEST_RESULT=$STF_PASS

echo "--INFO: Validate a non-existent file"
...
# Test #5: validate a directory

typeset -i TEST_RESULT=$STF_PASS

echo "--INFO: Validate a directory"
...
svccfg validate $STF_SUITE > $OUTFILE 2>$ERRFILE
ret=$?
[[ $ret -ne 1 ]] &&  {
	echo "--DIAG: [${assertion}, test 5]
	svccfg validate expected to return 1, got $ret"
...
# Test #6: validate a device file

typeset -i TEST_RESULT=$STF_PASS

echo "--INFO: Validate a device file"
device="/dev/random"

svccfg validate $device > $OUTFILE 2>$ERRFILE
ret=$?
[[ $ret -ne 1 ]] &&  {
...
# Test #7: validate a profile 

typeset -i TEST_RESULT=$STF_PASS

echo "--INFO: Validate a profile file"

profile=validate_003_profile.xml

svccfg validate $profile> $OUTFILE 2>$ERRFILE
ret=$?
[[ $ret -ne 1 ]] &&  {
	echo "--DIAG: [${assertion}, test 7]
	svccfg validate expected to return 1, got $ret"
