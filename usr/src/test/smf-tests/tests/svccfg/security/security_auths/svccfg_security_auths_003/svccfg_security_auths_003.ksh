...
#
# ASSERTION: svccfg_security_auths_003
#
# DESCRIPTION:
#	A user having only the 'solaris.smf.modify.dependency'
#	authorization can create, delete or modify property groups
#	of type 'dependency', but not of other types.
#
...
# Assertion ID
readonly assertion=svccfg_security_auths_003
...
typeset retmsg=""
typeset expmsg=""
readonly tested_authorization="'solaris.smf.modify.dependency'"

# Begin test
extract_assertion_info ${me}

# ---------------------------------------------------------------------------- #
# PART 1: dependency-type property groups can be created
echo "--INFO: PART 1:"
echo "	Users having only the ${tested_authorization} auth
	can create dependency-type property groups"
...
addpg ${GL_TEST_SVCPG}_new dependency
EOF)
rc=$?
expmsg=""
...
if [[ $rc != 0 ]] || [[ "$retmsg" != "$expmsg" ]]; then
	...
	echo "--DIAG: svccfg failed to add dependency-type pg 
		${GL_TEST_SVCPG}_new
...
addpg ${GL_TEST_INSTPG}_new dependency
EOF)
rc=$?
expmsg=""
if [[ $rc != 0 ]] || [[ "$retmsg" != "$expmsg" ]]; then
...
# ---------------------------------------------------------------------------- #
# PART 2: Existing props in dependency pgs can be modified by users with
#	  'solaris.smf.modify.dependency'
echo "--INFO: PART 2:"
echo "	Users having only the ${tested_authorization} auth
	can modify existing properties in dependency-type pgs"
...
if [[ $rc != 0 ]] || [[ "$retmsg" != "$expmsg" ]]; then
	tmp_result=$STF_FAIL
	# Could not modify pg
	echo "--DIAG: svccfg failed to modify pg ${GL_TEST_SVCPG}_dependency
	EXPECTED: rc = 0, error = '$expmsg'
	RETURNED: rc = $rc, error = '$retmsg'"
fi
...
# ---------------------------------------------------------------------------- #
# PART 3: New properties can be added to dependency pgs by users with the
#	  ${tested_authorization} auth
echo "--INFO: PART 3:"
echo "	Users having the ${tested_authorization} auth
	can add new properties can be added to dependency-type pgs"
...
if [[ $rc != 0 ]] || [[ "$retmsg" != "$expmsg" ]]; then
	tmp_result=$STF_FAIL
	# Could not modify pg
	echo "--DIAG: svccfg failed to add pg ${GL_TEST_INSTPG}_dependency
	EXPECTED: rc = 0, error = '$expmsg'
	RETURNED: rc = $rc, error = '$retmsg'"
fi
...
if [[ $tmp_result != $STF_FAIL ]]; then
	...
	typeset propval=$(echo "
			select svc:/${GL_TEST_SERVICE}:${GL_TEST_INSTANCE}
			listprop ${GL_TEST_INSTPG}_dependency/modby" | \
			svccfg -f - 2>/dev/null | \
			awk '{ printf $3 }')
	rc=$?
	expmsg="${GL_TEST_USERNAME}"
...
# ---------------------------------------------------------------------------- #
# PART 4: dependency-type pgs can be deleted with ${tested_authorization} auth
echo "--INFO: PART 4:"
echo "	Users having only the ${tested_authorization} auth
	can delete dependency-type pgs from the repository"
...
# ---------------------------------------------------------------------------- #
# PART 5: Non-dependency pgs cannot be created by users having only the 
#	  ${tested_authorization} auth
echo "--INFO: PART 5:"
echo "	Users having only the ${tested_authorization} auth
	cannot create non-dependency type pgs"

tmp_result=$STF_PASS

# Test svc-pgs only
# Run through all other types of pgs, including user-defined types
for pgtype in method framework application userdefined; do
	retmsg=$(svccfg -f - 2>&1 1>/dev/null <<EOF
select ${GL_TEST_SERVICE}
...
#	  values in non-dependency pgs, including pgs of user-defined types
echo "--INFO: PART 6:"
echo "	Users having only the ${tested_authorization} auth
	cannot change values in non-dependency type pgs"
...
#	  properties to non-dependency pgs, including pgs of user-defined types
echo "--INFO: PART 7:"
echo "	Users having only the ${tested_authorization} auth
	cannot add new props to non-dependency type pgs"
...
#	  pgs of non-dependency type, including pgs of user-defined types
echo "--INFO: PART 8:"
echo "	Users having only the ${tested_authorization} auth
	cannot delete non-dependency type pgs"
