...
#
# ASSERTION: svccfg_security_auths_004
#
# DESCRIPTION:
#	A user possessing only the 'solaris.smf.modify.application'
#	authorization can create, delete or modify property groups
#	of type 'application', but not of other types.
#
...
# Assertion ID
readonly assertion=svccfg_security_auths_004
...
typeset retmsg=""
typeset expmsg=""
readonly tested_authorization="'solaris.smf.modify.application'"

# Begin test
extract_assertion_info ${me}

# ---------------------------------------------------------------------------- #
# PART 1: application-type property groups can be created
echo "--INFO: PART 1:"
echo "	Users having only the ${tested_authorization} auth
	can create application-type property groups"
...
addpg ${GL_TEST_SVCPG}_new application
EOF)
rc=$?
expmsg=""
...
if [[ $rc != 0 ]] || [[ "$retmsg" != "$expmsg" ]]; then
	...
	echo "--DIAG: svccfg failed to add application-type pg 
		${GL_TEST_SVCPG}_new
...
addpg ${GL_TEST_INSTPG}_new application
EOF)
rc=$?
expmsg=""
if [[ $rc != 0 ]] || [[ "$retmsg" != "$expmsg" ]]; then
...
# ---------------------------------------------------------------------------- #
# PART 2: Existing props in application pgs can be modified by users with
#	  'solaris.smf.modify.application'
echo "--INFO: PART 2:"
echo "	Users having only the ${tested_authorization} auth
	can modify existing properties in application-type pgs"
...
if [[ $rc != 0 ]] || [[ "$retmsg" != "$expmsg" ]]; then
	tmp_result=$STF_FAIL
	# Could not modify pg
	echo "--DIAG: svccfg failed to modify pg ${GL_TEST_SVCPG}_application
	EXPECTED: rc = 0, error = '$expmsg'
	RETURNED: rc = $rc, error = '$retmsg'"
fi
...
# ---------------------------------------------------------------------------- #
# PART 3: New properties can be added to application pgs by users with the
#	  ${tested_authorization} auth
echo "--INFO: PART 3:"
echo "	Users having the ${tested_authorization} auth
	can add new properties can be added to application-type pgs"
...
retmsg=$(svccfg -f - 2>&1 1>/dev/null <<EOF
select svc:/${GL_TEST_SERVICE}:${GL_TEST_INSTANCE}
setprop ${GL_TEST_INSTPG}_application/modby = astring: "${GL_TEST_USERNAME}"
EOF)
rc=$?
expmsg=""
if [[ $rc != 0 ]] || [[ "$retmsg" != "$expmsg" ]]; then
	tmp_result=$STF_FAIL
	# Could not modify pg
	echo "--DIAG: svccfg failed to add pg ${GL_TEST_INSTPG}_application
	EXPECTED: rc = 0, error = '$expmsg'
	RETURNED: rc = $rc, error = '$retmsg'"
fi
...
if [[ $tmp_result != $STF_FAIL ]]; then
	...
	typeset propval=$(echo "
			select svc:/${GL_TEST_SERVICE}:${GL_TEST_INSTANCE}
			listprop ${GL_TEST_INSTPG}_application/modby" | \
			svccfg -f - 2>/dev/null | \
			awk '{ printf $3 }')
	rc=$?
	expmsg="${GL_TEST_USERNAME}"
...
# ---------------------------------------------------------------------------- #
# PART 4: application-type pgs can be deleted with ${tested_authorization} auth
echo "--INFO: PART 4:"
echo "	Users having only the ${tested_authorization} auth
	can delete application-type pgs from the repository"
...
# ---------------------------------------------------------------------------- #
# PART 5: Non-application pgs cannot be created by users having only the 
#	  ${tested_authorization} auth
echo "--INFO: PART 5:"
echo "	Users having only the ${tested_authorization} auth
	cannot create non-application type pgs"

tmp_result=$STF_PASS

# Test svc-pgs only
# Run through all other types of pgs, including user-defined types
for pgtype in method framework dependency userdefined; do
	retmsg=$(svccfg -f - 2>&1 1>/dev/null <<EOF
select ${GL_TEST_SERVICE}
...
#	  values in non-application pgs, including pgs of user-defined types
echo "--INFO: PART 6:"
echo "	Users having only the ${tested_authorization} auth
	cannot change values in non-application type pgs"
...
#	  properties to non-application pgs, including pgs of user-defined types
echo "--INFO: PART 7:"
echo "	Users having only the ${tested_authorization} auth
	cannot add new props to non-application type pgs"
...
#	  pgs of non-application type, including pgs of user-defined types
echo "--INFO: PART 8:"
echo "	Users having only the ${tested_authorization} auth
	cannot delete non-application type pgs"
