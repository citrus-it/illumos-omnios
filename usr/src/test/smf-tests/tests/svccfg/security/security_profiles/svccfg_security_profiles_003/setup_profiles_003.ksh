...
#
# If repository setup was successful, add the "Service Operator" profile
#
[[ $result == $STF_PASS ]] && \
	user_attr_mod add "profiles" "Service Operator"
rc=$?
[[ $rc -ne $STF_PASS ]] && result=$STF_UNRESOLVED

# Append the tested authorization name into the post-setup file
cat >> ${f_postsetup} <<EOF
export tested_auth="'Service Operator' profile"
EOF
