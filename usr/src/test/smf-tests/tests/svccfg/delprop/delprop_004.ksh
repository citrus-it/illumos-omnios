...
#
# ASSERTION: svccfg_delprop_004
#
# DESCRIPTION:
#	If the property name specified in the 'delprop pg/name' 
#	subcommand does not exist in the selected entity then a 
#	diagnostic message is sent to stderr and the subcommand 
#	will exit with a status of 1.
#
# STRATEGY:
#	Verify with the following conditions:
#	#1: delete a property that was never defined
#	#2: delete a property that was already deleted within the svccfg 
#	    sessions.
#	#3: delete a property that was already deleted within another session.
#
...
# Assertion ID
readonly assertion=svccfg_delprop_004
...
EXP_ERRMSG="svccfg \($CMDFILE, line 2\): No such property $TEST_PROPERTY/foo"
if ! egrep -s "$EXP_ERRMSG" $ERRFILE
then
	echo "--DIAG: [${assertion}, test 1]
	EXPECTED: \"$EXP_ERRMSG\"
	OBSERVED: \"$(cat $ERRFILE)\""
...
echo "--INFO: Starting $assertion, test 2 (delete already deleted property"
...
cat << EOF > $CMDFILE
select $TEST_SERVICE
setprop $TEST_PROPERTY/foo = astring: "bar"
EOF
...
# Verify that message in stderr - non-fatal error
EXP_ERRMSG="svccfg \($CMDFILE, line 3\): No such property $TEST_PROPERTY/foo"
if ! egrep -s "$EXP_ERRMSG" $ERRFILE
then
	echo "--DIAG: [${assertion}, test 2]
	EXPECTED: \"$EXP_ERRMSG\"
...
echo "--INFO: Starting $assertion, test 3 (delete already deleted property"
echo "		in different sessions"
