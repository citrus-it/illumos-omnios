...
#
# ASSERTION: svccfg_select_004
#
# DESCRIPTION:
#	If the (valid) FMRI specified in the subcommand "select FMRI"
#	refers to an object that does not exist in the repository,
...
# Assertion ID
readonly assertion=svccfg_select_004
...
function cleanup {
	...
	service_delete $TEST_SERVICE_FMRI > /dev/null 2>&1

	service_exists ${TEST_SERVICE_FMRI}
	[[ $? -eq 0 ]] && {
...
svccfg select ${TEST_SERVICE_FMRI} > $OUTFILE 2>$ERRFILE
ret=$?
...
EMSG="svccfg: Pattern '$TEST_SERVICE_FMRI' $NO_MATCH"
...
[[ $? -ne 0 ]] && {
	...
	service $TEST_SERVICE_FMRI  should exist in
