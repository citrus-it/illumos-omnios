...
#
# ASSERTION: svccfg_select_002
#
# DESCRIPTION:
#	The subcommand "select fmri" selects the entity whose absolute
#	or relative fmri has been specified by the fmri parameter.  On
#	success, no error is seen and the exit status is 0
#
# STRATEGY:
#       No separate test case is required for this assertion; it will be
#	tested by other test cases that use the "select" clause.
#
...
# Assertion ID
readonly assertion=svccfg_select_002
...
function cleanup {
        ...
        # Note that $TEST_SERVICE_FMRI may or may not exist so don't check
	# results.  Just make sure the service is gone.
	typeset i=1
	while [[ $i -le $num_entities ]]  
	do
		service_delete ${TEST_SERVICE_FMRI}_$i > /dev/null 2>&1

		service_exists ${TEST_SERVICE_FMRI}_$i
		[[ $? -eq 0 ]] && {
			echo "--DIAG: [${assertion}, cleanup] 
			service ${TEST_SERVICE_FMRI} should not exist in
...
do
	...
	SERVICE_FMRI=${TEST_SERVICE_FMRI}_$index	
	NSERVICE_FMRI=${TEST_SERVICE_FMRI}_${nindex}
	NSERVICE=${TEST_SERVICE}_${nindex}
	INSTANCE=${TEST_SERVICE}_${index}_${TEST_INSTANCE}_${index}

	#  Create svccfg file that select an non-existent instance
	cat <<EOF >$CMDFILE
	select ${SERVICE_FMRI}
	select ${NSERVICE_FMRI}
	list ${NSERVICE}*
	select ${SERVICE_FMRI}:${INSTANCE}
	list 
	end
EOF
