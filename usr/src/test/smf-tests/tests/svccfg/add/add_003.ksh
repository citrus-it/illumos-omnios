...
#
# ASSERTION: svccfg_add_003
#
# DESCRIPTION:
#	Calling the 'add name' subcommand with 'name' being an 
#	entity that was previously deleted (with 'delete name') 
#	will add the entity.  If no errors have occurred during 
...
#
# Delete the service.  If this fails consider it a fatal error
#
svccfg delete $TEST_SERVICE > $OUTFILE 2>$ERRFILE
ret=$?
[[ $ret -ne 0 ]] && {
	echo "--DIAG: [${assertion}]
	error deleting service $TEST_SERVICE
	error output is $(cat $ERRFILE)"
...
[[ $ret -ne 0 ]] && {
	echo "--DIAG: [${assertion}]
	error re-adding service $TEST_SERVICE" 
