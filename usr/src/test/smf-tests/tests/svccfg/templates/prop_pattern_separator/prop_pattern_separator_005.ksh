...
#
# ASSERTION: prop_pattern_separator_005
#
# DESCRIPTION:
#	Verify the property separator pattern when the explicitly setting
#	all the attributes when separator is set to a number (1).
#
# STRATEGY:
...
readonly registration_file=/tmp/prop_pattern_separator_005.xml
...
manifest_generate $registration_template \
	...
	PROPSEPARATOR="$PROP_SEPARATOR" > $registration_file

manifest_purgemd5 $registration_file

svccfg -v import $registration_file >$OUTFILE 2>$ERRFILE
ret=$?
if [ $ret -ne 0 ]
then
	echo "-- DIAG: [${assertion}]" \
...
verify_prop $test_service $pgn/internal_separators astring $PROP_SEPARATOR
