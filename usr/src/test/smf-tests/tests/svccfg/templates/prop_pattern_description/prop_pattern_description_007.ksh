...
#
# ASSERTION: prop_pattern_description_007
#
# DESCRIPTION:
...
readonly registration_file=/tmp/prop_pattern_description_007.xml
...
while [ $i -lt 4093 ]
do
	sz=`echo "$DESCRIPTION\c" | wc -c | sed -e 's/ //g'`
	echo "--- INFO : Verify description of size $sz is valid"

	manifest_generate $registration_template \
		DESC_LANG_1="'$DESC_LANG'" \
		DESCRIPTION_1="$DESCRIPTION" \
		DESC_LANG_2="'ES'" \
		DESCRIPTION_2="spanish description" > $registration_file
	...
	verify_prop QUIET $test_service $pgn/description_$DESC_LANG ustring $DESCRIPTION
	...
	DESCRIPTION=${DESCRIPTION}${DESC_ORIG}
	i=`expr $i + 256`
done
