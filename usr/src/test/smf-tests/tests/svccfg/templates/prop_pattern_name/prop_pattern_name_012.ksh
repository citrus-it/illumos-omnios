...
#
# ASSERTION: prop_pattern_name_012
#
# DESCRIPTION:
#	Verify the property name pattern when the explicitly setting
#	all the attributes when the name is set to values that fit
#	the valid patterns.
#
# STRATEGY:
...
readonly registration_file=/tmp/prop_pattern_name_012.xml
...
PG_NAME="foo"
PROP_NAMES="f_foo f-foo f_foo,bar f.foo,bar f-foo,bar"
PROP_NAMES="$PROP_NAMES f1,bar foo123 foo,bar123"

for PROP_NAME in $PROP_NAMES
do
	echo "--INFO : Verify that property name $PROP_NAME succeeds"
	...
	if [ $? -ne 0 ]; then
		echo "-- DIAG: $PROP_NAME svccfg import failed"
		echo "$OUTFILE :"
		cat $OUTFILE
		echo "$ERRFILE :"
		cat $ERRFILE
		...
		echo "-- INFO: $PROP_NAME svccfg import succeeded"

		echo "--INFO: Validate the property pattern name"
		pgn=${PROP_PREFIX_NT}${PG_NAME}_${PROP_NAME}
		verify_prop $test_service $pgn/name astring $PROP_NAME
