...
#
# ASSERTION: pg_pattern_required_009
#
# DESCRIPTION:
#	Verify the property group name pattern when the explicitly setting
#	all the attributes, required is false, but no type is set.
#
# STRATEGY:
...
readonly registration_file=/tmp/pg_pattern_required_009.xml
...
manifest_generate $registration_template \
	GROUPNAME="name='$PG_NAME'" \
	GROUPTYPE="type='application'" \
	PGNAME="name='$PG_NAME'" \
	PGTYPE="" \
	PGTARGET="target='instance'" \
	PGREQUIRED="required='$PG_REQUIRED'" > $registration_file
...
pgn=${PG_PREFIX_N}${PG_NAME}
verify_prop $test_service $pgn/required boolean $PG_REQUIRED
