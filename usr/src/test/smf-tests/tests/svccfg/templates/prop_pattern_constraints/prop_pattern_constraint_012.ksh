...
#
# ASSERTION: prop_pattern_constraint_016
#
# DESCRIPTION:
#	Verify the property name pattern when the explicitly setting
#	all the attributes with multiple value entries.
#
# STRATEGY:
...
readonly registration_file=/tmp/prop_pattern_constraint_016.xml
...
PROP_NAME="bar"
PROP_VAL=`$NAME_GEN 8`
PROP_VALUE="<value name='$PROP_VAL' />"
CNT=1
NPROP_VAL=$PROP_VAL
NAMES=$PROP_VAL
while [ $CNT -le 64 ]
do
	MULTILINE_PROPVAL="\$PROP_VALUE"
	manifest_generate $registration_template \
		PROPNAME="name='$PROP_NAME'" \
		PROPTYPE="type='astring'" \
		PROPREQUIRED="required='false'" \
		PROPVALUE="$MULTILINE_PROPVAL" \
		PROPRANGE="" > $registration_file
	...
	echo "verify_multi $test_service $pgn/constraint_name ${NAMES}"
	verify_multi $test_service $pgn/constraint_name ${NAMES}
	...
	while [ $LNCNT -ne 8 ]
	do
		eval NPROP_VAL=`$NAME_GEN 8`

		len=`echo "$NPROP_VAL\c" | wc -c`
		NAMES="$NPROP_VAL $NAMES"
		PROP_VALUE="${PROP_VALUE}\\
						<value name='${NPROP_VAL}' />"
