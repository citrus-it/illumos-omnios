...
#
# ASSERTION: prop_pattern_constraint_012
#
# DESCRIPTION:
#	Verify the property name pattern when the explicitly setting
#	all the attributes, when both the value and range min/max is
#	set.
#
# STRATEGY:
...
readonly registration_file=/tmp/prop_pattern_constraint_012.xml
...
manifest_generate $registration_template \
	...
	PROPVALUE="<value name='$PROP_VALUE' />" \
	PROPRANGE="<range min='$PROP_RANGEMIN' max='$PROP_RANGEMAX' />" > $registration_file
...
echo "--INFO: Validate the property pattern constraint when setting attributes"
pgn=${PROP_PREFIX_NT}${PG_NAME}_${PROP_NAME}
verify_prop $test_service $pgn/constraint_name astring ${PROP_VALUE}
verify_prop $test_service $pgn/constraint_range astring ${PROP_RANGEMIN},${PROP_RANGEMAX}
