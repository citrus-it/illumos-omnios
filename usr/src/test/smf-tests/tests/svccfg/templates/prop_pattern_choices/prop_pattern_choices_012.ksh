...
#
# ASSERTION: prop_pattern_choices_012
#
# DESCRIPTION:
#	Verify the property name pattern when the explicitly setting
#	all the attributes, when there are multiple choice value lines.
#
# STRATEGY:
...
readonly registration_file=/tmp/prop_pattern_choices_012.xml
...
PG_NAME="foo"
PROP_NAME="bar"
CHOICE_VAL=`$NAME_GEN 8`
CHOICE_VALUE="<value name='$CHOICE_VAL' />"
CNT=1
NCHOICE_VAL=$CHOICE_VAL
NAMES=$CHOICE_VAL
while [ $CNT -le 64 ]
do
	MULTILINE_CHOICEVAL="\$CHOICE_VALUE"
	manifest_generate $registration_template \
		...
		PROPVALUES="" \
		CONSTRAINTS="" \
		CHOICEVALUE="$MULTILINE_CHOICEVAL" \
		CHOICERANGE="" \
	...
	echo "verify_multi $test_service $pgn/choices_name ${NAMES}"
	verify_multi $test_service $pgn/choices_name ${NAMES}
	...
	while [ $LNCNT -ne 8 ]
	do
		eval NCHOICE_VAL=`$NAME_GEN 8`

		len=`echo "$NCHOICE_VAL\c" | wc -c`
		NAMES="$NCHOICE_VAL $NAMES"
		CHOICE_VALUE="${CHOICE_VALUE}\\
						<value name='${NCHOICE_VAL}' />"
