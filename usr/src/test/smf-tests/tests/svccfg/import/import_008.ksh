...
#
# ASSERTION: svccfg_import_008
#
# DESCRIPTION:
#	Calling the 'import' subcommand with a valid service manifest 
#	that was previously applied (with the same services and
#	instances) will result in *no* changes to any of the existing
#	repository information, if the override property is not set.
#
#
# STRATEGY:
...
readonly registration_template=$MYLOC/import_008.xml
...
manifest_generate $registration_template \
	...
	SERVICE_APP=$SERVICE_APP  \
 	LOGFILE=$LOGFILE \
	STATEFILE=$STATEFILE | sed 's/TESTDATA/first/' >$registration_file


manifest_purgemd5 $registration_file

cp $registration_file /tmp/1.xml
...
[[ $ret -ne 0 ]] && {
       	echo "--DIAG: [${assertion}]
	service $TEST_SERVICE:$TEST_INSTANCE not diabled as expected"
...
manifest_generate $registration_template \
	...
	SERVICE_APP=$SERVICE_APP  \
 	LOGFILE=$LOGFILE \
	STATEFILE=$STATEFILE | sed 's/TESTDATA/second/' >$registration_file
...
[[ $ret -ne 0 ]] &&  {
	echo "--DIAG: [${assertion}]
	svccfg mport expected to return 0, got $ret"
...
cmp -s /tmp/export.$$.1 /tmp/export.$$.2
ret=$?
[[ $ret -ne 0 ]] && {
       	echo "--DIAG: [${assertion}]
	repository unexpectedly changed after 2nd import.
	The output after the first import is:
	$(cat /tmp/export.$$.1)
	The output after the second import is:
	$(cat /tmp/export.$$.2)"
