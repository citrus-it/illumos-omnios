...
#
# ASSERTION: svccfg_listpg_006
#
# DESCRIPTION:
#	The 'listpg *' subcommand produces output identical to the
#	'listpg' subcommand.  If no errors have occurred during
...
function cleanup {
	...
	rm -f $ERRFILE $OUTFILE $CMDFILE $ERRFILE2 $OUTFILE2
...
svccfg -f $CMDFILE > $OUTFILE2 2>$ERRFILE2
ret=$?
...
[[ -s $ERRFILE2 ]] &&  {
	echo "--DIAG: [${assertion}]
	stderr not expected, but got $(cat $ERRFILE2)"
...
[[ $? -ne 0 ]] && {
	echo "--DIAG: [${assertion}]
	output of two listpg calls different . . ."

	echo "Output of 'listpg *': $(cat $OUTFILE2)"
	echo "Output of 'listpg': $(cat $OUTFILE)"
