...
#
# ASSERTION: svccfg_addpg_005
#
# DESCRIPTION:
#	If an invalid type is given to the 'addpg' subcommand
#	then a diagnostic message is sent to stderr and the return
#	status is set to 1.
#
# STRATEGY:
#	This assertion has two subtests:
#	1) add a pg of an invalid type to a service
#	2) add a pg of an an invalid type to an instance
#
...
cat <<EOF>$CMDFILE
select ${TEST_SERVICE}
addpg ${TEST_PROPERTY} ${INVALID_NAME} 
EOF
...
#
svcprop -p $INVALID_NAME $TEST_SERVICE_FMRI > /dev/null 2>&1
ret=$?
[[ $ret -eq 0 ]] &&  {
	echo "--DIAG: [${assertion}, test 1]
