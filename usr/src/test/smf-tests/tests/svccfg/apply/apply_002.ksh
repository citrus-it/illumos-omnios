...
#
# ASSERTION: svccfg_apply_002
#
# DESCRIPTION:
#	Calling the "apply file" subcommand where the file contains a
#	valid service profile will result in the service instances
#	specified within the file being disabled, if the services are
#	specified to be disabled.  If no errors have occurred during
#	processing, there is nothing seen on stderr and the
#	command exist status is 0.
#
# STRATEGY:
#	This test uses a service that is existing in the repository.  
#	- Create and import a test service, with enabled=true
#	   (at this point the service is up and running)
#	- Create a test profile with the test service enabled (enabled=true)
#	- Apply the profile 
#	   (this should have no impact on the state of the services)
#	- Update the profile such that enable is not true (enabled=false)
#	- Apply the profile 
#	   (this should disable the service)
#
# NOTE:  Results in UNRESOLVED due to 1000566
#	
...
manifest_generate $manifest_template \
	...
	SERVICE_APP=$SERVICE_APP  \
 	LOGFILE=$LOGFILE \
	STATEFILE=$STATEFILE | sed 's/ENABLE_VALUE/false/' >$manifest_file
...
[[ $ret -ne 0 ]] && {
       	echo "--DIAG: [${assertion}]
	$TEST_SERVICE:$TEST_INSTANCE did not transition to disabled as expected" 
	RESULT=$(update_result $STF_UNRESOLVED $RESULT)
	exit $RESULT
}
