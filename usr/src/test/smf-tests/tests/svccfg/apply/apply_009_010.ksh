...
#
# ASSERTION: svccfg_apply_009
#
# DESCRIPTION:
#	Calling the 'apply' subcommand with a valid service profile that
#	contains services that are unknown will ignore the service (i.e.
#	the service will not be added to the repository).
#
# STRATEGY:
#	- Create a test profile with an unknown test service
#	- Apply the profile 
#	- Verify that the repository does not know about the service
#
#	
...
# start __stf_assertion__
#
# ASSERTION: svccfg_apply_010
#
# DESCRIPTION:
#	Calling the 'apply' subcommand with a valid service profile that
#	contains services that are unknown to the repostory will ??
#
# STRATEGY:
#	- Create a test profile with the test service disabled (enabled=false)
#	- Apply the profile 
#	- Create and import the test service, with enabled=true
#
# NOTE: It isn't clear what the behavior of the test should do.  A bug will
#	be filed.
#	
# end __stf_assertion__
###############################################################################
...
# 
# Test 9: Apply a non-existent service
#

assertion=svccfg_apply_009

echo "--INFO: Starting $assertion, apply non-existent service"
...
# 
# Test 10:  Import the "non-existent" service that was applied.
#

assertion=svccfg_apply_010

echo "--INFO: Starting $assertion, import service"
...
manifest_generate $profile_template \
	...
	TEST_INSTANCE=$TEST_INSTANCE  | sed 's/ENABLE_VALUE/false/' >$profile_file
...
[[ $RESULT != $STF_PASS ]] && exit $RESULT

#
# Test #10: Add the non-existent service that was just imported
#


echo "--INFO: Starting $assertion, test 10 (add service)"
typeset -i TEST_RESULT=$STF_PASS
...
[[ $ret -ne 0 ]] && {
	echo "--DIAG: [${assertion}]
	service $TEST_SERVICE should exist in repository 
	but does not"
