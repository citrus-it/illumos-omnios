...
#
# ASSERTION: svccfg_delpg_003
#
# DESCRIPTION:
#	If an invalid name is given to the 'delpg' subcommand then
#	a diagnostic message sent to stderr and an exit status of 1.
#
...
#
# Attempt to delete an invalid  property group to the service
#
cat <<EOF>$CMDFILE
select ${TEST_SERVICE}
delpg ${INVALID_NAME} 
...
if ! egrep -s "$INVALID_PROPGRP_ERRMSG" $ERRFILE
then
	echo "--DIAG: [${assertion}]
	Expected error message \"$INVALID_PROPGRP_ERRMSG\"
	but got \"$(cat $ERRFILE)\""
