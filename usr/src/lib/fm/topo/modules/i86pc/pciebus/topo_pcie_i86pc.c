    ...
    const smbios_struct_t *sp, void *arg)
{
	...
	case SMB_TYPE_PROCESSOR: {
		...
		entry->spe_label = topo_mod_strdup(sc->sc_mod,
		    info.smbi_location);
	...
	case SMB_TYPE_OBDEVEXT: {
		...
		entry->sde_label = topo_mod_strdup(sc->sc_mod, ob.smboe_name);
	...
	case SMB_TYPE_SLOT: {
		...
		entry->sde_label = topo_mod_strdup(sc->sc_mod, slot.smbl_name);
...
privdata_free(topo_mod_t *mod, mod_pcie_privdata_t *pd)
{
	...
	if (pd->mpp_procs != NULL) {
		for (i = 0; i < pd->mpp_nprocs; i++) {
			topo_mod_strfree(mod,
			    __DECONST(char *, pd->mpp_procs[i].spe_label));
		}
	...
	if (pd->mpp_obdevs != NULL) {
		for (i = 0; i < pd->mpp_nobdevs; i++) {
			topo_mod_strfree(mod,
			    __DECONST(char *, pd->mpp_obdevs[i].sde_label));
		}
	...
	if (pd->mpp_slots != NULL) {
		for (i = 0; i < pd->mpp_nslots; i++) {
			topo_mod_strfree(mod,
			    __DECONST(char *, pd->mpp_slots[i].sde_label));
		}
