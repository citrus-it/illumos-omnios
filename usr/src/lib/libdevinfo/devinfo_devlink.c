...
/*
 * CRC32 wrapper around zlib, which is also delivered in /lib and so is
 * safe for use here. Calls may be chained by passing the previous return
 * value as the initial crc.
 */
static uint32_t
 ...
 * read-only so that any into the mapping before it is unmapped faults
