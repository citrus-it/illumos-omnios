 ...
 * might have guessed given the request structures, but all of these objects
 * which represent something are opaque. Each library handle is independent of
 * one another and each controller handle is independent of one another. In
 * general, it is expected that only a single controller handle is used at a
 * given time for a given library handle, but this is not currently enforced.
 * Error information and parallelism is tied into this, see 'Parallelism, Thread
    ...
    nvme_ns_t **nsp)
{
	...
	*ctrlp = NULL;
	 ...
	 * We don't want to conflate 001 and 1 as the same here.
	 */
	if (ctrl_namelen == 4) {
		return (nvme_error(nvme, NVME_ERR_BAD_CONTROLLER, 0,
	...
	if (name[4] == '0' && name[5] == '0') {
		return (nvme_error(nvme, NVME_ERR_BAD_CONTROLLER, 0,
		    "multiple leading zeros aren't allowed for the instance "
		    "specified in %.*s", (int)ctrl_namelen, name));
	}
