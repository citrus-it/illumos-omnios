 ...
 * process authenticating on the terminal within one login session shares it,
 * regardless of the session of the process performing the authentication. In
 * particular, the pfexec authentication helper runs as a session leader of its
 * own new session. Fall back to the caller's own session for a terminal which
 * is not currently a controlling terminal.
 */
static pid_t
