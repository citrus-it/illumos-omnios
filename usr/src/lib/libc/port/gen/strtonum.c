...
/*
 * Common epilogue for the strtonum() family: publish the error string,
 * set errno for the error case or restore the caller's saved value for
 * success, and tell the caller whether the conversion failed.
 */
static int
    ...
    unsigned long long maxval, const char **errstrp, int base)
{
	...
	if (minval > maxval) {
		 ...
		 * the unsigned complement of the value. That is the kind of
		 * surprise these functions exist to prevent, so reject
		 * negative input outright.
		 */
		error = TOOSMALL;
	} else {
