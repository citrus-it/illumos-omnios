...
spawn_attrs_apply(const spawn_param_t *sp)
{
	...
	if (spa->sa_psflags & POSIX_SPAWN_SETSID) {
		lwp->lwp_errno = 0;
		if (setpgrp(PGRPSYS_SETSID, 0, 0) < 0 && lwp->lwp_errno != 0)
			return (lwp->lwp_errno);
	}
...
/*
 * Build the path name for the next attempt in a PATH search.
 * Returns the remainder of the search path or NULL if we're done.
 */
static const char *
...
spawn_exec(kspawn_param_t *ksp)
{
	...
	do {
		...
		if (err == ENOEXEC) {
			 ...
			 * recognised format. As execvp() does, execute
			 * it as a shell script and stop the search here.
			 */
...
spawn_main(void *arg)
{
	...
	err = spawn_attrs_apply(ksp->ksp_param);
	if (err == 0)
		err = spawn_factions_apply(ksp);
	if (err == 0) {
		err = spawn_exec(ksp);
		execfail = (err != 0);
	}
...
spawn(void *path, void *sparam, uint32_t spsize, void *sargs, uint32_t sasize)
{
	...
	/*
	 * If cfork() succeeds, wait for the child to apply the spawn
	 * attributes and file actions and attempt the exec. The wait is not
	 * interruptible but it is bounded - every child exit path signals
	 * completion, including abnormal termination in proc_exit().
	 */
	mutex_enter(&ksp->ksp_lock);
