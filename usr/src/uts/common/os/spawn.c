 ...
 * child set no CLDEVAPORATE, so it becomes an ordinary zombie,
 ...
 * the highest kept descriptor, the second allocates an appropriately
 * sized table and copies the descriptors. Each keep decision is
 * re-evaluated under the entry lock in the second pass, so a
 * descriptor that a sibling thread changes between the passes is
 * treated as though the change had happened before the spawn.
 *
 ...
 * flight.
 *
 * A process that is still being built has no address space of its own
 * (p_as == kas) and /proc reports any such process, including one
 * part way through an ordinary exec, as a system process. Spawn adds
 * nothing new here. Separately, while SSPAWNING is set the /proc
 * control operations fail with EBUSY.
 *
 ...
 * token and, subject to the AUDIT_ARGV and AUDIT_ARGE policies, the
 * argument and environment vectors are included. One nuance is that
 * the path recorded is ksp_path, which for posix_spawnp() is the name
 * the child actually exec'd after the PATH search, not necessarily
