...
retry:
	 ...
	 * sympathy with the active lwp. A spawned child carries
	 * the flag harmlessly. Since stop() refuses a process
	 * that has has no address space of its own, it cannot
	 * fire until the child has exec'd, and the exec stop
	 * then consumes it.
	 */
	 ...
	 * they return back to user level. The spawning lwp is
	 * excluded for spawn(2). It remains blocked within the
	 * system call until the child execs, and its interruptible
	 * wait there would honour a pending directed stop
	 * prematurely. It stops on exit from spawn(2) in the usual
	 * way once the child's exec has been resolved.
	 */
