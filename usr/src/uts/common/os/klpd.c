...
/*
 * Open the calling process's controlling terminal in the same way as an open
 * of /dev/tty. As with /dev/tty, no access check is made against the
 * underlying terminal device. A terminal which is a controlling terminal
 * cannot be cloned by this open, so the vnode is not changed by VOP_OPEN().
 * Returns a file pointer which is not associated with any file descriptor, or
 * NULL on failure.
 */
static file_t *
...
again:
	...
	if (ttyfp != NULL) {
		 ...
		 * reference held here is unaffected by the result of the
		 * upcall and it is always released below.
		 */
 ...
 * pam_timestamp(7) stamps, held for the calling process's user and controlling
 * terminal. A process without a controlling terminal has no cached state to
 * discard, and no privilege is required since the caller can only ever discard
 * state for its own user and terminal.
 */
int
