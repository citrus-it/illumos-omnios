...
flist_spawn(uf_info_t *pfip, uf_info_t *cfip, const kspawn_param_t *ksp)
{
	...
	for (cnfiles = 1; cnfiles <= maxkept; cnfiles = (cnfiles << 1) | 1)
		continue;
...
/*
 * In-kernel equivalent of fcntl(ofd, F_DUP2FD, nfd). This is used by the
 * F_DUP2FD family of fcntl(2) commands and to apply spawn(2) file actions
 * in a spawned child.
 */
int
fdup2(int ofd, int nfd)
{
	...
	if (ofd == nfd) {
		...
		/*
		 * As required by posix_spawn_file_actions_adddup2(), a dup2
		 * file action whose descriptors are equal clears FD_CLOEXEC
		 * and FD_CLOFORK on the descriptor, where dup2() itself
		 * would do nothing.
		 */
