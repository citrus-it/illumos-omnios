 ...
 * PRIV_PROC_SESSION. The caller must also hold an open descriptor for the
 * terminal. PRIV_PROC_SESSION is in the basic set by default but, unlike
 * stronger candidates such as PRIV_SYS_DEVICES, it is available within
 * non-global zones, where this interface is needed by pfexecd. Sites which
 * remove it from the basic set restrict grafting along with the other
