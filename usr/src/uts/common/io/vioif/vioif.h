 ...
 * to submit it. A buffer moves from FREE to INFLIGHT when it is allocated
...
typedef struct vioif_ctrlbuf {
	...
	/*
	 * The location of the current request's ack byte within "cb_dma",
	 * used to report the outcome of an abandoned request when its
...
struct vioif {
	 ...
	 * control buffer is returned to the free list.
	 */
