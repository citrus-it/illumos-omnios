...
static bool	asyischar(cons_polledio_arg_t);
...
asy_update_from_modem_status(struct asycom *asy)
{
	...
	struct asyncline *async = asy->asy_priv;
	int t_cflag = async->async_ttycommon.t_cflag;
	const bool term_uses_hw_flowctl = (t_cflag & CRTSCTS) != 0;

	/*
	 * We examine the modem status register state in a loop to make
	 * sure that we don't miss events.  If modem state is sampled
	 * on the falling edge of the interrupt line and used as the basis
	 * for detecting status changes, we would miss an event occurring
	 * after we read the MSR.
	 *
	 * Note that reading the MSR resets the modem status interrupt state.
	 */
	for (int msr = asy_get(asy, ASY_MSR);
	    ASY_MSR_DELTAS(msr) != 0;
		...
		bool cts_changed = ((asy->asy_msr ^ msr) & ASY_MSR_CTS) != 0;
		if (term_uses_hw_flowctl && cts_changed) {
			async_flowc_action flow =
			    (msr & ASY_MSR_CTS) != 0 ? FLOW_START : FLOW_STOP;
