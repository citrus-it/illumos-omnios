...
typedef struct virtio_ops {
	...
	uint16_t	(*vop_queue_size_get)(virtio_t *);
	void		(*vop_queue_size_set)(virtio_t *, uint16_t);
	uint64_t	(*vop_queue_noff_get)(virtio_t *);
	bool		(*vop_queue_enable_get)(virtio_t *);
	void		(*vop_queue_enable_set)(virtio_t *, bool);
	void		(*vop_queue_addr_set)(virtio_t *, uint64_t, uint64_t,
			    uint64_t);
	...
	void		(*vop_msix_queue_set)(virtio_t *, uint16_t);
	uint16_t	(*vop_msix_queue_get)(virtio_t *);
...
#define	VIRTIO_MODERN_MIN_CAPS		4
#define	VIRTIO_CAP_MAX			VPC_DEVICE_CFG
#define	VIOCAP(vio, x) (&(vio)->vio_caps[(x) - 1])
	virtio_pci_cap_t		vio_caps[VIRTIO_CAP_MAX];
