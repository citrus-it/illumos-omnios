...
virtio_queue_free(virtio_queue_t *viq)
{
	...
	/*
	 * Sever any vector sharing relationships in both directions. Queues
 ...
 * called; a queue handler must therefore tolerate being called when its own
 * queue has no work, just as with the shared fixed interrupt fallback. Any
 * number of queues may share the vector of one owning queue, but sharing
 * relationships must not be chained. This function may only be called during
 * the initialisation phase, and both queues must have been configured with
 * handler functions.
 */
void
...
/*
 * The queue whose vector this queue's interrupts arrive on: the owning queue
 * when the vector is shared, or the queue itself otherwise.
 */
