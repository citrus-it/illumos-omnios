...
virtio_legacy_devcfg_get8(virtio_t *vio, uintptr_t offset)
{
	...
	uint8_t r = virtio_get_leg8(vio, vio->vio_config_offset + offset);
...
{
	mutex_enter(&vio->vio_mutex);
	uint16_t r = virtio_get_leg16(vio, vio->vio_config_offset + offset);
	mutex_exit(&vio->vio_mutex);
...
{
	mutex_enter(&vio->vio_mutex);
	uint32_t r = virtio_get_leg32(vio, vio->vio_config_offset + offset);
	mutex_exit(&vio->vio_mutex);
...
{
	mutex_enter(&vio->vio_mutex);
	virtio_put_leg8(vio, vio->vio_config_offset + offset, value);
	mutex_exit(&vio->vio_mutex);
}
...
{
	mutex_enter(&vio->vio_mutex);
	virtio_put_leg16(vio, vio->vio_config_offset + offset, value);
	mutex_exit(&vio->vio_mutex);
}
...
{
	mutex_enter(&vio->vio_mutex);
	virtio_put_leg32(vio, vio->vio_config_offset + offset, value);
	mutex_exit(&vio->vio_mutex);
}
...
static uint16_t
virtio_legacy_queue_size_get(virtio_t *vio)
{
	return (virtio_get_leg16(vio, VIRTIO_LEGACY_QUEUE_SIZE));
}

static void
virtio_legacy_queue_addr_set(virtio_t *vio, uint64_t descaddr,
...
{
	return (virtio_get_leg8(vio, VIRTIO_LEGACY_ISR_STATUS));
}
...
static void
virtio_legacy_msix_queue_set(virtio_t *vio, uint16_t msi)
{
	virtio_put_leg16(vio, VIRTIO_LEGACY_MSIX_QUEUE, msi);
}

static uint16_t
virtio_legacy_msix_queue_get(virtio_t *vio)
{
	return (virtio_get_leg16(vio, VIRTIO_LEGACY_MSIX_QUEUE));
}
...
static bool
virtio_legacy_queue_enable_get(virtio_t *vio)
{
...
static void
virtio_legacy_queue_enable_set(virtio_t *vio, bool enable)
{
	/* Legacy queues are always enabled */
}

static uint64_t
virtio_legacy_queue_noff_get(virtio_t *vio)
{
	/* Legacy queues don't use notification offsets */
	return (0);
}
...
virtio_ops_t virtio_legacy_ops = {
	...
	.vop_queue_noff_get = virtio_legacy_queue_noff_get,
