...
{
	return (ddi_get8(VIOCAP(vio, VPC_COMMON_CFG)->vpc_barh,
	    (uint8_t *)(VIOCAP(vio, VPC_COMMON_CFG)->vpc_bar + offset)));
}

static inline uint16_t
virtio_get_cmn16(virtio_t *vio, uintptr_t offset)
{
	return le16toh((ddi_get16(VIOCAP(vio, VPC_COMMON_CFG)->vpc_barh,
	    (uint16_t *)(VIOCAP(vio, VPC_COMMON_CFG)->vpc_bar + offset))));
}

static inline uint32_t
virtio_get_cmn32(virtio_t *vio, uintptr_t offset)
{
	return le32toh((ddi_get32(VIOCAP(vio, VPC_COMMON_CFG)->vpc_barh,
	    (uint32_t *)(VIOCAP(vio, VPC_COMMON_CFG)->vpc_bar + offset))));
}

static inline void
virtio_put_cmn8(virtio_t *vio, uintptr_t offset, uint8_t value)
{
	ddi_put8(VIOCAP(vio, VPC_COMMON_CFG)->vpc_barh,
	    (uint8_t *)(VIOCAP(vio, VPC_COMMON_CFG)->vpc_bar + offset), value);
}

static inline void
virtio_put_cmn16(virtio_t *vio, uintptr_t offset, uint16_t value)
{
	ddi_put16(VIOCAP(vio, VPC_COMMON_CFG)->vpc_barh,
	    (uint16_t *)(VIOCAP(vio, VPC_COMMON_CFG)->vpc_bar + offset),
	    htole16(value));
}
...
{
	ddi_put32(VIOCAP(vio, VPC_COMMON_CFG)->vpc_barh,
	    (uint32_t *)(VIOCAP(vio, VPC_COMMON_CFG)->vpc_bar + offset),
	    htole32(value));
}
...
{
	return (ddi_get8(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_barh,
	    (uint8_t *)(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_bar + offset)));
}

static uint16_t
virtio_modern_devcfg_get16(virtio_t *vio, uintptr_t offset)
{
	return (le16toh(ddi_get16(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_barh,
	    (uint16_t *)(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_bar + offset))));
}

static uint32_t
virtio_modern_devcfg_get32(virtio_t *vio, uintptr_t offset)
{
	return (le32toh(ddi_get32(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_barh,
	    (uint32_t *)(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_bar + offset))));
}
...
virtio_modern_devcfg_get64(virtio_t *vio, uintptr_t offset)
{
	...
	do {
		...
		val = (uint64_t)le32toh(virtio_modern_devcfg_get32(vio,
		    offset + sizeof (uint32_t))) << 32;
		val |= le32toh(virtio_modern_devcfg_get32(vio, offset));
...
{
	ddi_put8(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_barh,
	    (uint8_t *)(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_bar + offset), value);
}

static void
virtio_modern_devcfg_put16(virtio_t *vio, uintptr_t offset, uint16_t value)
{
	ddi_put16(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_barh,
	    (uint16_t *)(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_bar + offset),
	    htole16(value));
}
...
{
	ddi_put32(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_barh,
	    (uint32_t *)(VIOCAP(vio, VPC_DEVICE_CFG)->vpc_bar + offset),
	    htole32(value));
}
...
{
	ddi_put16(VIOCAP(vio, VPC_NOTIFY_CFG)->vpc_barh,
	    (uint16_t *)(VIOCAP(vio, VPC_NOTIFY_CFG)->vpc_bar + offset),
	    htole16(value));
}
...
{
	return (ddi_get8(VIOCAP(vio, VPC_ISR_CFG)->vpc_barh,
	    (uint8_t *)(VIOCAP(vio, VPC_ISR_CFG)->vpc_bar + offset)));
}
...
virtio_modern_device_set_features(virtio_t *vio, uint64_t features)
{
	...
	/*
	 * We now need to check that this bit is still set in the features. If
...
static uint16_t
virtio_modern_queue_size_get(virtio_t *vio)
{
	return (virtio_get_cmn16(vio, VIRTIO_MODERN_COMMON_Q_SIZE));
}

static void
virtio_modern_queue_size_set(virtio_t *vio, uint16_t qsz)
{
	return (virtio_put_cmn16(vio, VIRTIO_MODERN_COMMON_Q_SIZE, qsz));
}

static void
virtio_modern_queue_addr_set(virtio_t *vio, uint64_t descaddr,
...
static void
virtio_modern_msix_queue_set(virtio_t *vio, uint16_t msi)
{
	virtio_put_cmn16(vio, VIRTIO_MODERN_COMMON_Q_MSIX, msi);
}

static uint16_t
virtio_modern_msix_queue_get(virtio_t *vio)
{
	return (virtio_get_cmn16(vio, VIRTIO_MODERN_COMMON_Q_MSIX));
}
...
static bool
virtio_modern_queue_enable_get(virtio_t *vio)
{
	return (virtio_get_cmn16(vio, VIRTIO_MODERN_COMMON_Q_ENABLE) != 0);
}

static void
virtio_modern_queue_enable_set(virtio_t *vio, bool enable)
{
	virtio_put_cmn16(vio, VIRTIO_MODERN_COMMON_Q_ENABLE, enable);
}

static uint64_t
virtio_modern_queue_noff_get(virtio_t *vio)
{
	uint64_t noff;
