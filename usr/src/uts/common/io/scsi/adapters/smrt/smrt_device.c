...
smrt_map_device(smrt_t *smrt)
{
	 ...
	 * registers".  The Intelligent I/O (I2O) Architecture Specification
	 * describes this somewhat more coherently as "the memory region
	 * specified by the first base address configuration register
	 * indicating memory space
	 */
	rnumber = pci_bar_find_type(dip, PCI_BARTYPE_MEM, NULL);
	if (rnumber == -1) {
