 ...
 * during attach:
 *
 *    o Entry size: 128B is preferred, matching the behaviour of the
 *      reference Linux and FreeBSD drivers. Larger sizes (192B,
 *      256B) are used only if 128B is not supported. Each SQ entry
 *      is this size, comprising descriptors followed by inline
 *      packet header data.
 *
...
/*
 * Negotiate LLQ parameters with the device and send SET_FEATURE to configure
 * it.
 */
static bool
ena_configure_llq(ena_t *ena)
{
	...
	/* Negotiate header location - we require inline header */
	...
	/* Negotiate entry size, preferring 128B as the reference drivers do */
	supported = llq_feat->efllq_entry_size_ctrl_supported;
	if (supported & ENAHW_LLQ_ENTRY_SIZE_128B) {
	...
	/* Negotiate number of descriptors before header */
	...
	} else if (supported & ENAHW_LLQ_NUM_DESCS_BEFORE_HEADER_1) {
		ena->ena_llq_num_descs_before_header =
		    ENAHW_LLQ_NUM_DESCS_BEFORE_HEADER_1;
