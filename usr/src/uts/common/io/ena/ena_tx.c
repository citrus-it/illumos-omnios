 ...
 * membar_producer() in ena_submit_tx() guarantees all write-combining
 * stores have been flushed before the doorbell reaches the device,
 * so there is no need to write phase bits in reverse order or
...
ena_submit_tx(ena_txq_t *txq, uint16_t desc_idx)
{
	...
	/*
	 * Ensure all preceding writes are visible before the doorbell.
	 * In LLQ mode this flushes the write-combining stores to
	 * device memory; in host placement mode it orders the
	 * descriptor stores before the doorbell.
	 */
	membar_producer();
