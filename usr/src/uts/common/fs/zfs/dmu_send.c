...
static int dmu_krrp_direct_arc_read(dmu_sendarg_t *dsp, const blkptr_t *bp);
    ...
    const zbookmark_phys_t *zb)
{
	...
	if (!dsp->sendsize && dsp->dsa_krrp_task != NULL &&
	    !BP_IS_EMBEDDED(bp)) {
		rc = dmu_krrp_direct_arc_read(dsp, bp);
		if (rc == 0 || rc != ENODATA)
			return (rc);
	}
...
static int
dmu_krrp_direct_arc_read(dmu_sendarg_t *dsp, const blkptr_t *bp)
{
	int error;
	...
	error = arc_io_bypass(dsp->dsa_os->os_spa, bp,
	    dmu_krrp_arc_bypass, &bypass);
	if (error == 0) {
		DTRACE_PROBE(krrp_send_arc_bypass);
	} else if (error == ENODATA || error == EAGAIN) {
