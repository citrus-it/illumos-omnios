...
typedef struct kspawn_param {
	bool		ksp_complete;
	int		ksp_error;	/* error reported back by the child */
	kmutex_t	ksp_lock;
	kcondvar_t	ksp_cv;
	...
	/*
	 * Results of pre-scanning the file actions, used to limit the copy of
	 * the parent's file descriptor table; see spawn_prescan().
	 */
	int		ksp_closefrom;	/* lowest closefrom() bound */
	uint_t		ksp_nreffds;
	int		*ksp_reffds;	/* fds consumed as action sources */
} kspawn_param_t;
