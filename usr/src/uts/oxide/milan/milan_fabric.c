 ...
 */
uint8_t
milan_ioms_n_pcie_cores(const uint8_t nbno)
{
...
milan_pcie_core_info(const uint8_t iomsno, const uint8_t coreno)
{
	...
	return (&milan_lane_maps[iomsno * 2 + coreno]);
}
...
milan_fabric_ioms_init(zen_ioms_t *ioms)
{
	 ...
	 */
	ioms->zio_npcie_cores = milan_ioms_n_pcie_cores(iomsno);
...
milan_fabric_nbif_syshub_dma(zen_nbif_t *nbif)
{
	 ...
	 */
	if (nbif->zn_nbifno == 1 && nbif->zn_ioms->zio_num <= 1) {
