 ...
 * IOHUB[0,2] have 22 bridges and those on IOHUB[1,3] have 9, the configuration
 * of the first 9 is common across both so we can get away with a single table.
 */
static const zen_ioapic_info_t turin_ioapic_routes[IOAPIC_NROUTES_IOAPIC0] = {
...
CTASSERT(ARRAY_SIZE(turin_ioapic_routes) == IOAPIC_NROUTES_IOAPIC0);
    ...
    const uint16_t reginst)
{
	...
	case SMN_UNIT_IOAPIC:
		reg = turin_ioapic_smn_reg(iunit, def, reginst);
		break;
	case SMN_UNIT_IOHC:
...
turin_fabric_ioapic(zen_ioms_t *ioms)
{
	smn_reg_t reg;
	uint32_t val;
	const uint_t nroutes = (ioms->zio_num % 2 == 0) ?
	    IOAPIC_NROUTES_IOAPIC0 :
	    IOAPIC_NROUTES_IOAPIC1;
