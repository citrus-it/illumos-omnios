    ...
    uint32_t mask)
{
	...
	case PCIE_LINK_SPEED_8:
		...
		eq_val = PCIE_PORT_LC_EQ_CTL_8GT_SET_GO_TO_EQ(eq_val, 1);
		break;
	case PCIE_LINK_SPEED_16:
		...
		eq_val = PCIE_PORT_LC_EQ_CTL_16GT_SET_GO_TO_EQ(eq_val, 1);
		break;
	case PCIE_LINK_SPEED_32:
		...
		eq_val = PCIE_PORT_LC_EQ_CTL_32GT_SET_GO_TO_EQ(eq_val, 1);
		break;
	default:
		return (EINVAL);
	}
	 ...
	 * redo at this rate so the new mask takes effect on the next retrain.
	 * The hardware clears these once equalisation has run.
	 */
