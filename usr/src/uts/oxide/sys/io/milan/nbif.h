 ...
 */
#define	MILAN_IOHC_MAX_NBIF		3
#define	MILAN_IOHC_MAX_NBIF_ALT		2
/*
    ...
    const uint8_t nbifno, const uint8_t devno, const uint8_t funcno)
{
	 ...
	 */
#ifdef	DEBUG
	const uint8_t MILAN_NBIF_FNVALID[MILAN_IOHC_MAX_NBIF]
	...
	ASSERT3U(nbio32, <, 4);
	ASSERT3U(nbif32, <, MILAN_IOHC_MAX_NBIF);
    ...
    const uint8_t nbifno, const uint16_t reginst)
{
	...
	ASSERT3U(nbio32, <, 4);
	ASSERT3U(nbif32, <, MILAN_IOHC_MAX_NBIF_ALT);
	ASSERT3U(nents, >, reginst32);
	ASSERT0(def.srd_reg & SMN_APERTURE_MASK);
