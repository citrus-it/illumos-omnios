...
genoa_cgpll_set_ssc(bool ssc)
{
	...
	reg = FCH_MISC_A_CGPLLCFG4_MMIO(fch_misc_a);
	val = mmio_reg_read(reg);
	val = FCH_MISC_A_CGPLLCFG4_SET_FRACN_OVR(val, 0x1333);
	mmio_reg_write(reg, val);

	reg = FCH_MISC_A_CGPLLCFG5_MMIO(fch_misc_a);
	val = mmio_reg_read(reg);
	val = FCH_MISC_A_CGPLLCFG4_SET_SLEWFRACN_OVR(val, 0x3a);
	mmio_reg_write(reg, val);
	...
	val = mmio_reg_read(reg);
	val = FCH_MISC_A_CLKCTL0_SET_FBDIV_LOADEN(val, 1);
	mmio_reg_write(reg, val);

	reg = FCH_MISC_A_CLKCTL2_MMIO(fch_misc_a);
	val = mmio_reg_read(reg);
	val = FCH_MISC_A_CLKCTL2_SET_FCH_400M_MASKOFF(val, 1);
	mmio_reg_write(reg, val);
