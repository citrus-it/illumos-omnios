...
ipcc_cb_read_espi(void *arg, uint8_t *buf, size_t *len)
{
	...
	return (espi_rx(ipcc->is_espi_block, 0xcc, buf, len));
}
...
ipcc_cb_write_espi(void *arg, uint8_t *buf, size_t *len)
{
	...
	return (espi_tx(ipcc->is_espi_block, 0xcc, buf, len));
}
...
ipcc_readable_espi(void *arg)
{
	...
	return (espi_readable(ipcc->is_espi_block));
}
...
ipcc_writable_espi(void *arg)
{
	...
	return (espi_writable(ipcc->is_espi_block));
}
    ...
    uint64_t timeout_ms)
{
	...
	if (ipcc->is_uart) {
		readable = ipcc_readable_uart;
		writable = ipcc_writable_uart;
	} else {
		readable = ipcc_readable_espi;
		writable = ipcc_writable_espi;
	}
...
ipcc_cb_open(void *arg)
{
	...
	if (ipcc->is_uart) {
		...
		if (err != 0) {
			...
			dev_err(ipcc_dip, CE_WARN,
			    "ldi open of '%s' failed, error %d",
			    ipcc_path, err);
			return (err);
		}

		/*
		 * Whilst there is nothing expected to be autopushed on the DWU
		 * UART, check and pop anything that is. This also allows
		 * easier testing on commodity hardware.
		 */
		...
		ipcc->is_espi_block = fch_espi_mmio_block();
		err = espi_acquire(ipcc->is_espi_block);
		if (err != 0) {
			ldi_ident_release(ipcc->is_ldiid);
			dev_err(ipcc_dip, CE_WARN,
			    "eSPI acquisition failed, error %d", err);
			return (err);
		}
	}
...
ipcc_ioctl(dev_t dev, int cmd, intptr_t data, int mode, cred_t *cr, int *rv)
{
	...
	ipcc.is_dev = dev;
	ipcc.is_cred = cr;
	ipcc.is_uart = (strcmp(ipcc_path, "/dev/espi0") != 0);
...
keyset_done:
	...
	case IPCC_ESPI_GETCONF: {	//XXX stub
		mmio_reg_block_t block = fch_espi_mmio_block();
		espi_get_configuration(block);
...
ipcc_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	if (oxide_board_data->obd_ipccmode == IPCC_MODE_ESPI_C) {
		ipcc_ops = &ipcc_ops_espi;
		ipcc_path = ddi_strdup("/dev/espi0", KM_SLEEP);
	} else {
		/*
	...
	ipcc_dbgmsg(NULL, 0, "Using device '%s'", ipcc_path);
