...
ptwalk_xc(xc_arg_t a1, xc_arg_t a2 __unused, xc_arg_t a3 __unused)
{
	...
	if (req->pr_write) {
		...
		/*
		 * Belt and braces. AMD state that the TW_CFG write is itself a
		 * complete invalidation, but let's do an explicit flush too.
		 */
...
ptwalk_tw_trial_bits(void)
{
	...
	v = AMD_TW_CFG_SET_DIS_NESTED_PDPE_CACHE(v, 1);
	v = AMD_TW_CFG_SET_DIS_NESTED_PML4E_CACHE(v, 1);
	v = AMD_TW_CFG_SET_DIS_GUEST_PDE_CACHE(v, 1);
	v = AMD_TW_CFG_SET_DIS_GUEST_PDPE_CACHE(v, 1);
	v = AMD_TW_CFG_SET_DIS_GUEST_PML4E_CACHE(v, 1);
	v = AMD_TW_CFG_SET_DIS_HOST_NATIVE_PDE_CACHE(v, 1);
	v = AMD_TW_CFG_SET_DIS_HOST_NATIVE_PDPE_CACHE(v, 1);
	v = AMD_TW_CFG_SET_DIS_HOST_NATIVE_PML4E_CACHE(v, 1);
	v = AMD_TW_CFG_SET_DIS_32K_PAGES(v, 1);
	v = AMD_TW_CFG_SET_DIS_DC_SPEC_WALK(v, 1);
	v = AMD_TW_CFG_SET_DIS_ALL_SPEC_WALK(v, 1);
...
/*
 * Variation 4, ours: no partial-walk caching at any level for native walks,
 * covering the PDPE and PML4E caches that 1a and 1b left enabled.
 */
static void
ptwalk_partial_cache_native(ktest_ctx_hdl_t *ctx)
{
	ptwalk_req_t req = { .pr_write = true };
	uint64_t v = 0;
	...
	req.pr_tw_set = v;
	req.pr_tw_clr = ptwalk_tw_trial_bits() & ~v;
	if (ptwalk_run(ctx, &req, "partial_cache_native"))
		KT_PASS(ctx);
}

/* Bit 10 alone, the host-native PDPE cache, to bisect variation 4. */
static void
ptwalk_pdpe_cache_native(ktest_ctx_hdl_t *ctx)
{
	ptwalk_req_t req = { .pr_write = true };

	req.pr_tw_set = AMD_TW_CFG_SET_DIS_HOST_NATIVE_PDPE_CACHE(0, 1);
	req.pr_tw_clr = ptwalk_tw_trial_bits() & ~req.pr_tw_set;
	if (ptwalk_run(ctx, &req, "pdpe_cache_native"))
		KT_PASS(ctx);
}

/* Bit 9 alone, the host-native PML4E cache, the other half of the bisection. */
static void
ptwalk_pml4e_cache_native(ktest_ctx_hdl_t *ctx)
{
	ptwalk_req_t req = { .pr_write = true };

	req.pr_tw_set = AMD_TW_CFG_SET_DIS_HOST_NATIVE_PML4E_CACHE(0, 1);
	req.pr_tw_clr = ptwalk_tw_trial_bits() & ~req.pr_tw_set;
	if (ptwalk_run(ctx, &req, "pml4e_cache_native"))
		KT_PASS(ctx);
}

/* AMD second round, priority 1: PDPE caches for native, guest and nested. */
static void
ptwalk_pdpe_cache_all(ktest_ctx_hdl_t *ctx)
{
	ptwalk_req_t req = { .pr_write = true };
	uint64_t v = 0;
	...
	req.pr_tw_set = v;
	req.pr_tw_clr = ptwalk_tw_trial_bits() & ~v;
	if (ptwalk_run(ctx, &req, "pdpe_cache_all"))
		KT_PASS(ctx);
}

/* AMD second round, priority 2: no speculative walks from the data side. */
static void
ptwalk_dc_spec_walk(ktest_ctx_hdl_t *ctx)
{
	ptwalk_req_t req = { .pr_write = true };

	req.pr_tw_set = AMD_TW_CFG_SET_DIS_DC_SPEC_WALK(0, 1);
	req.pr_tw_clr = ptwalk_tw_trial_bits() & ~req.pr_tw_set;
	if (ptwalk_run(ctx, &req, "dc_spec_walk"))
		KT_PASS(ctx);
}

/* AMD second round, priority 3: no speculative walks at all. */
static void
ptwalk_all_spec_walk(ktest_ctx_hdl_t *ctx)
{
	ptwalk_req_t req = { .pr_write = true };

	req.pr_tw_set = AMD_TW_CFG_SET_DIS_ALL_SPEC_WALK(0, 1);
	req.pr_tw_clr = ptwalk_tw_trial_bits() & ~req.pr_tw_set;
	if (ptwalk_run(ctx, &req, "all_spec_walk"))
		KT_PASS(ctx);
}

/*
 * AMD second round, priority 4: no partial-walk caching at any level in any
 * domain, TW_CFG bits 9 to 17.
 */
static void
ptwalk_partial_cache_all(ktest_ctx_hdl_t *ctx)
{
	ptwalk_req_t req = { .pr_write = true };
	uint64_t v = 0;
	...
	req.pr_tw_set = v;
	req.pr_tw_clr = ptwalk_tw_trial_bits() & ~v;
	if (ptwalk_run(ctx, &req, "partial_cache_all"))
		KT_PASS(ctx);
}
...
_init()
{
	...
	VERIFY0(ktest_add_test(ks, "3_no_coalesce", ptwalk_no_coalesce,
	    KTEST_FLAG_NONE));
	VERIFY0(ktest_add_test(ks, "4_partial_cache_native",
	    ptwalk_partial_cache_native, KTEST_FLAG_NONE));
	VERIFY0(ktest_add_test(ks, "5_pdpe_cache_native",
	    ptwalk_pdpe_cache_native, KTEST_FLAG_NONE));
	VERIFY0(ktest_add_test(ks, "6_pml4e_cache_native",
	    ptwalk_pml4e_cache_native, KTEST_FLAG_NONE));
	VERIFY0(ktest_add_test(ks, "7_pdpe_cache_all", ptwalk_pdpe_cache_all,
	    KTEST_FLAG_NONE));
	VERIFY0(ktest_add_test(ks, "8_dc_spec_walk", ptwalk_dc_spec_walk,
	    KTEST_FLAG_NONE));
	VERIFY0(ktest_add_test(ks, "9_all_spec_walk", ptwalk_all_spec_walk,
	    KTEST_FLAG_NONE));
	VERIFY0(ktest_add_test(ks, "10_partial_cache_all",
	    ptwalk_partial_cache_all, KTEST_FLAG_NONE));
