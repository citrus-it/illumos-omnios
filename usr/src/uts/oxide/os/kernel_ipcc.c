...
eb_ipcc_espi_read(void *arg, uint8_t *buf, size_t *len)
{
	...
	return (espi_rx(data->kid_espi_block, buf, len));
}
...
eb_ipcc_espi_write(void *arg, uint8_t *buf, size_t *len)
{
	...
	return (espi_tx(data->kid_espi_block, buf, len));
}
...
kernel_ipcc_init(ipcc_init_t stage)
{
	if (oxide_board_data->obd_ipccmode == IPCC_MODE_DISABLED) {
		//XXX XXX XXX XXX
		if (oxide_board_data->obd_board == OXIDE_BOARD_RUBYRED &&
		    stage == IPCC_INIT_EARLYBOOT) {
			extern int prom_debug;
			prom_debug = 1;
			eb_ipcc_pause(1000);
			mmio_reg_block_t block = fch_espi_mmio_block();
			(void) espi_init(block);
