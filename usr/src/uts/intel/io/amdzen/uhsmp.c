    ...
    int *rvalp)
{
	...
	if (ucmd.uc_id > uhsmp->uhsmp_maxfn) {
		dev_err(uhsmp->uhsmp_dip, CE_WARN,
		    "Invalid UHSMP function 0x%x (max 0x%x)", ucmd.uc_id,
		    uhsmp->uhsmp_maxfn);
		return (EINVAL);
	}
...
uhsmp_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 */
#define	UHSMP_TESTVAL	0x1234567
	uhsmp_cmd_t testcmd = {
		.uc_id = UHSMP_TESTMESSAGE,
		.uc_args[0] = UHSMP_TESTVAL
	};
	if ((ret = uhsmp_cmd(uhsmp, 0, &testcmd)) != 0) {
		dev_err(dip, CE_CONT, "?UHSMP test error %d\n", ret);
		goto err;
	}
	if (testcmd.uc_response != UHSMP_RESPONSE_OK ||
	    testcmd.uc_args[0] != UHSMP_TESTVAL + 1) {
	...
	switch (vercmd.uc_args[0]) {
	case 1:
		uhsmp->uhsmp_maxfn = 0x11;
		break;
	case 2:
		uhsmp->uhsmp_maxfn = 0x12;
		break;
	case 3:
		uhsmp->uhsmp_maxfn = 0x14;
		break;
	case 4:
		uhsmp->uhsmp_maxfn = 0x15;
		break;
	case 5:
		uhsmp->uhsmp_maxfn = 0x2f;
		break;
	case 7:
		uhsmp->uhsmp_maxfn = 0x3f;
		break;
	default:
		dev_err(dip, CE_WARN,
