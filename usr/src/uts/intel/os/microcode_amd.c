 ...
 */
static ucode_errno_t
ucode_copy_amd(const ucode_file_amd_t *ucodefp, size_t size,
    cpu_ucode_info_t *uinfop)
{
	...
	memcpy(uinfop->cui_pending_ucode, ucodefp, size);
...
ucode_locate_amd(cpu_t *cp, cpu_ucode_info_t *uinfop)
{
	...
	if (amd_ucodef == NULL) {
		...
		if (rc == EM_HIGHERREV)
			return (rc);
		if (rc == EM_OK) {
			return (ucode_copy_amd(amd_ucodef, amd_ucodef_len,
			    uinfop));
		}
	}
